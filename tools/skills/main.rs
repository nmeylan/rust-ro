            ...
            if let Some(job) = SHORT_CLASS_NAME.get(job_tree.name().to_lowercase().as_str()) {
                ...
                #[cfg(feature = "generate_override_stub")] {
                    ...
                    if is_self(skill_config) {
                        job_skills_file.write_all(format!("impl SelfSkill for {} {{\n", to_struct_name(skill_config)).as_bytes()).unwrap();
                        job_skills_file.write_all(b"}\n").unwrap();
                    }
...
fn write_skills(job_skills_file: &mut File, skill_config: &SkillConfig, item_name_ids: &HashMap<String, u32>) {
    ...
    if is_self(skill_config) {
        generate_is_self_skill(job_skills_file);
        generate_as_self_skill(job_skills_file);
    }
    if is_ground(skill_config) {
...
fn generate_is_self_skill(job_skills_file: &mut File) {
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file.write_all(b"    fn is_self_skill(&self) -> bool {\n").unwrap();
    job_skills_file.write_all(b"        true\n").unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
}

fn generate_as_self_skill(job_skills_file: &mut File) {
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file.write_all(b"    fn as_self_skill(&self) -> Option<&dyn SelfSkill> {\n").unwrap();
    job_skills_file.write_all(b"        Some(self)\n").unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn is_offensive(skill_config: &SkillConfig) -> bool {
    matches!(skill_config.target_type(), SkillTargetType::Attack) || skill_config.dmg_atk().is_some() || skill_config.dmg_atk_per_level().is_some()
}

fn is_passive(skill_config: &SkillConfig) -> bool {
    matches!(skill_config.target_type(), SkillTargetType::Passive)
}
...
fn is_support(skill_config: &SkillConfig) -> bool {
    matches!(skill_config.target_type(), SkillTargetType::Support)
}
