...
pub fn main() {
    ...
    let skills: SkillsConfig = serde_json::from_str(&fs::read_to_string(path).unwrap()).unwrap();


    generate_skills_impl(output_path, &skills);
    generate_skills_enum(output_path, &skills);
}

fn generate_skills_impl(output_path: &Path, skills: &SkillsConfig) {
...
fn generate_skills_enum(output_path: &Path, skills: &SkillsConfig) {
