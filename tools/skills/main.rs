...
pub fn main() {
    ...
    if !skill_tree_path.exists() {
        panic!(
            "config/skill_tree.json file does not exists at {}",
            skill_tree_path.to_str().unwrap()
        );
    }
    ...
    generate_skills_impl(
        output_path,
        &skills,
        &skill_tree,
        &mut skills_already_generated,
        &mut jobs_with_skills,
        &items_name_id,
    );
    generate_skills_enum(
        output_enum_path,
        &skills,
        &skill_tree,
        &skills_already_generated,
        &jobs_with_skills,
    );
    generate_skills_enum_to_object(output_path, &skills, &skill_tree, &skills_already_generated, &jobs_with_skills);
}

fn generate_skills_impl(
    output_path: &Path,
    skills: &Vec<SkillConfig>,
    skill_tree: &Vec<JobSkillTree>,
    skills_already_generated: &mut BTreeSet<String>,
    jobs_with_skills: &mut BTreeSet<String>,
    item_name_ids: &BTreeMap<String, u32>,
) {
    ...
    mod_file_base
        .write_all("// Generated by tools/skills/main.rs\n".to_string().as_bytes())
        .unwrap();
    mod_file_base
        .write_all("// Auto generated file do not edit manually\n\n".to_string().as_bytes())
        .unwrap();
    ...
    for job_tree in skill_tree.iter() {
        ...
        for skill in job_tree.tree().iter() {
            ...
            if let Some(job) = SHORT_CLASS_NAME.get(job_tree.name().to_lowercase().as_str()) {
                if (!skill.name().to_lowercase().starts_with(job) && !job_tree.name().to_lowercase().eq("bard"))
                    || (job_tree.name().to_lowercase().eq("bard")
                        && !(skill.name().to_lowercase().starts_with("bd") || skill.name().to_lowercase().starts_with("ba")))
                {
                    println!("Skipping skill {} for job {}", skill.name(), job_tree.name());
                    continue;
                }
                let skill_config = get_skill_config(skill.name(), skills)
    ...
    // let mut file =
    // File::create(output_path.join("skills").join("any.rs")).unwrap();
...
fn write_skills(job_skills_file: &mut File, skill_config: &SkillConfig, item_name_ids: &BTreeMap<String, u32>) {
    job_skills_file
        .write_all(format!("// {} - {}\n", skill_config.name, skill_config.description).as_bytes())
        .unwrap();

    generate_struct(job_skills_file, skill_config);

    job_skills_file
        .write_all(format!("impl SkillBase for {} {{\n", to_struct_name(skill_config)).as_bytes())
        .unwrap();
    ...
    if is_offensive(skill_config) {
        job_skills_file
            .write_all(format!("impl OffensiveSkillBase for {} {{\n", to_struct_name(skill_config)).as_bytes())
            .unwrap();
    ...
    if is_support(skill_config) {
        job_skills_file
            .write_all(format!("impl SupportiveSkillBase for {} {{\n", to_struct_name(skill_config)).as_bytes())
            .unwrap();
        job_skills_file.write_all(b"}\n").unwrap();
    }
    if is_interactive(skill_config) {
        job_skills_file
            .write_all(format!("impl InteractiveSkillBase for {} {{\n", to_struct_name(skill_config)).as_bytes())
            .unwrap();
        job_skills_file.write_all(b"}\n").unwrap();
    }
    if is_ground(skill_config) {
        job_skills_file
            .write_all(format!("impl GroundSkillBase for {} {{\n", to_struct_name(skill_config)).as_bytes())
            .unwrap();
        job_skills_file.write_all(b"}\n").unwrap();
    }
    if is_performance(skill_config) {
        job_skills_file
            .write_all(format!("impl PerformanceSkillBase for {} {{\n", to_struct_name(skill_config)).as_bytes())
            .unwrap();
        job_skills_file.write_all(b"}\n").unwrap();
    }
    if is_passive(skill_config) {
        job_skills_file
            .write_all(format!("impl PassiveSkillBase for {} {{\n", to_struct_name(skill_config)).as_bytes())
            .unwrap();
        job_skills_file.write_all(b"}\n").unwrap();
    }
}
...
fn generate_new(job_skills_file: &mut File, skill_config: &SkillConfig) {
    job_skills_file
        ...
        .unwrap();
    job_skills_file
        .write_all(format!("        if level > {} {{ return None }}\n", skill_config.max_level()).as_bytes())
        .unwrap();
    job_skills_file
        .write_all(b"        Some(Self { level, cast_time: 0, after_cast_act_delay: 0, after_cast_walk_delay: 0 })\n")
        .unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn generate_getters(job_skills_file: &mut File, skill_config: &SkillConfig) {
    ...
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn _after_cast_act_delay(&self) -> u32 {\n")
        .unwrap();
    job_skills_file.write_all(b"        self.after_cast_act_delay\n").unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn _after_cast_walk_delay(&self) -> u32 {\n")
        .unwrap();
    ...
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn _update_cast_time(&mut self, new_value: u32) {\n")
        .unwrap();
    job_skills_file.write_all(b"        self.cast_time = new_value;\n").unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn _update_after_cast_act_delay(&mut self, new_value: u32) {\n")
        .unwrap();
    job_skills_file
        .write_all(b"        self.after_cast_act_delay = new_value;\n")
        .unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn _update_after_cast_walk_delay(&mut self, new_value: u32) {\n")
        .unwrap();
    job_skills_file
        .write_all(b"        self.after_cast_walk_delay = new_value;\n")
        .unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
    ...
    if requirements.is_none() || (requirements.unwrap().sp_cost().is_none() && requirements.unwrap().sp_cost_per_level().is_none()) {
        ...
        generate_return_per_level_u32(
            job_skills_file,
            requirements.map(|c| c.sp_cost()).unwrap_or(&None),
            requirements.map(|c| c.sp_cost_per_level()).unwrap_or(&None),
        );
    }

    job_skills_file.write_all(b"    }\n").unwrap();
    job_skills_file
        .write_all(b"    fn _target_type(&self) -> SkillTargetType {\n")
        .unwrap();
    job_skills_file
        .write_all(format!("        SkillTargetType::{:?}\n", skill_config.target_type()).as_bytes())
        .unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
    ...
    job_skills_file
        .write_all(
            format!(
                "        {}\n",
                skill_config.damage_flags().unwrap_or(0) & SkillDamageFlags::IsMagical.as_flag() > 0
            )
            .as_bytes(),
        )
        .unwrap();
    ...
    job_skills_file
        .write_all(
            format!(
                "        {}\n",
                skill_config.damage_flags().unwrap_or(0) & SkillDamageFlags::IsPhysical.as_flag() > 0
            )
            .as_bytes(),
        )
        .unwrap();
...
fn generate_validate_sp(job_skills_file: &mut File, skill_config: &SkillConfig) {
    ...
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn _validate_sp(&self, status: &StatusSnapshot) -> SkillRequirementResult<u32> {\n")
        .unwrap();
    let requirements = skill_config.requires().as_ref();
    generate_validate_per_level(
        job_skills_file,
        "status.sp()",
        requirements.map(|c| c.sp_cost()).unwrap_or(&None),
        requirements.map(|c| c.sp_cost_per_level()).unwrap_or(&None),
...
fn generate_validate_hp(job_skills_file: &mut File, skill_config: &SkillConfig) {
    ...
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn _validate_hp(&self, status: &StatusSnapshot) -> SkillRequirementResult<u32> {\n")
        .unwrap();
    let requirements = skill_config.requires().as_ref();
    generate_validate_per_level(
        job_skills_file,
        "status.hp()",
        requirements.map(|c| c.hp_cost()).unwrap_or(&None),
        requirements.map(|c| c.hp_cost_per_level()).unwrap_or(&None),
    );
    job_skills_file.write_all(b"    }\n").unwrap();
}
    ...
    if let Some(requirements) = skill_config.requires() {
        if let Some(ammo_amount) = requirements.ammo_amount() {
            if let Some(ammo_flags) = requirements.ammo_flags() {
                job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
                job_skills_file
                    .write_all(b"    fn _validate_ammo(&self, character_ammo: Option<(AmmoType, u32)>) -> SkillRequirementResult<u32> {\n")
                    .unwrap();
                job_skills_file
                    .write_all(b"        if let Some(ammo_and_amount) = character_ammo {\n")
                    .unwrap();
                job_skills_file
                    .write_all(
                        format!(
                            "            if ammo_and_amount.1 >= {} && ({} & ammo_and_amount.0.as_flag()) > 0 {{ Ok({}) }} else {{ \
                             Err(()) }}\n",
                            ammo_amount, ammo_flags, ammo_amount
                        )
                        .as_bytes(),
                    )
                    .unwrap();
                ...
                panic!(
                    "Skill ({}) configuration has ammo amount requirement but no ammo flags",
                    skill_config.name
                );
            }
        }
...
fn generate_validate_state(job_skills_file: &mut File, skill_config: &SkillConfig) {
    if let Some(requirements) = skill_config.requires() {
        if let Some(state) = requirements.state() {
            job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
            job_skills_file
                .write_all(b"    fn _validate_state(&self, status: &StatusSnapshot) -> SkillRequirementResult<()> {\n")
                .unwrap();
...
fn generate_validate_zeny(job_skills_file: &mut File, skill_config: &SkillConfig) {
    ...
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn _validate_zeny(&self, status: &StatusSnapshot) -> SkillRequirementResult<u32> {\n")
        .unwrap();
    generate_validate_per_level(
        job_skills_file,
        "status.zeny()",
        requirements.map(|c| c.zeny_cost()).unwrap_or(&None),
        requirements.map(|c| c.zeny_cost_per_level()).unwrap_or(&None),
    );
    job_skills_file.write_all(b"    }\n").unwrap();
}
        ...
        if !requirements.item_cost().is_empty() {
                ...
                .write_all(
                    format!(
                        ...
                        requirements
                            ...
                            .iter()
                            .map(|item| format!(
                                "(NormalInventoryItem {{item_id: {}, name_english: \"{}\".to_string(), amount: {}}})",
                                item_name_ids
                                    .get(item.item())
                                    .unwrap_or_else(|| panic!("Item {} not found", item.item())),
                                item.item(),
                                item.amount()
                            ))
                            .collect::<Vec<String>>()
            ...
            for item in requirements.item_cost().iter() {
                job_skills_file
                    .write_all(
                        format!(
                            "        if !inventory.iter().any(|item| item.item_id == {} && item.amount >= {}) {{\n",
                            item_name_ids.get(item.item()).unwrap(),
                            item.amount()
                        )
                        .as_bytes(),
                    )
                ...
                if item.item().eq("Red_Gemstone") {
                    job_skills_file
                        .write_all(b"            return Err(UseSkillFailure::RedGemstone);\n")
                        .unwrap();
                } else if item.item().eq("Blue_Gemstone") {
                    job_skills_file
                        .write_all(b"            return Err(UseSkillFailure::BlueGemstone);\n")
                        .unwrap();
                } else if item.item().eq("Holy_Water") {
                    job_skills_file
                        .write_all(b"            return Err(UseSkillFailure::Holywater);\n")
                        .unwrap();
                } else {
                    job_skills_file
                        .write_all(b"            return Err(UseSkillFailure::NeedItem);\n")
                        .unwrap();
                }
                job_skills_file.write_all(b"        }\n").unwrap();
            }
...
fn generate_validate_weapon(job_skills_file: &mut File, skill_config: &SkillConfig) {
    if let Some(requirements) = skill_config.requires() {
        if let Some(weapon) = requirements.weapon_flags() {
            job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
            job_skills_file
                .write_all(b"    fn _validate_weapon(&self, status: &StatusSnapshot) -> SkillRequirementResult<()> {\n")
                .unwrap();
            job_skills_file
                .write_all(b"        if let Some(character_weapon) = status.right_hand_weapon() {\n")
                .unwrap();
            job_skills_file
                .write_all(
                    format!(
                        "            if {} & character_weapon.weapon_type().as_flag() > 0 {{ Ok(()) }} else {{ Err(()) }}\n",
                        weapon
                    )
                    .as_bytes(),
                )
...
fn generate_skip_validation_item(job_skills_file: &mut File, skill_config: &SkillConfig) {
    if let Some(skip_requirement) = skill_config.skip_requires() {
        if let Some(item_cost) = skip_requirement.item_cost() {
            job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
            job_skills_file
                .write_all(b"    fn _skip_item_validation(&self, state: Option<u64>) -> bool {\n")
                .unwrap();
            if let Some(state) = item_cost.state() {
                job_skills_file
                    ...
                    .unwrap();
                job_skills_file
                    .write_all(format!("        if state.unwrap_or(0) & {} > 0 {{ return true; }}\n", state.as_flag()).as_bytes())
                    .unwrap();
            }
            job_skills_file.write_all(b"        false\n").unwrap();
            job_skills_file.write_all(b"    }\n").unwrap();
...
fn generate_client_type(job_skills_file: &mut File, skill_config: &SkillConfig) {
    ...
    if matches!(skill_config.target_type(), SkillTargetType::Target) {
        job_skills_file
            .write_all(format!("        {}\n", SkillTargetType::Friend.value()).as_bytes())
            .unwrap();
    } else {
        job_skills_file
            .write_all(format!("        {}\n", skill_config.target_type().value()).as_bytes())
            .unwrap();
    }
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn generate_element(job_skills_file: &mut File, skill_config: &SkillConfig) {
    ...
    job_skills_file
        .write_all(format!("        Element::{:?}\n", skill_config.element().unwrap_or(Element::Neutral)).as_bytes())
        .unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn generate_inflict_status(job_skills_file: &mut File, skill_config: &SkillConfig) {
    ...
    if skill_config.bonus_to_target_before_hit().is_empty() {
        ...
        let change_to_inflict_status_effects = skill_config
            .bonus_to_target_before_hit()
            .iter()
        ...
        if change_to_inflict_status_effects.clone().count() == 0 {
            ...
            // We need to group per effect, because skills like meteor assault can cause
            // multiple effect
            ...
            for bonus in change_to_inflict_status_effects {
                match bonus.value.0 {
                    BonusType::ChanceToInflictStatusOnAttackPercentage(effect, _) => {
                        map.entry(format!("{:?}", effect)).or_insert_with(Vec::new).push(bonus.clone());
                    }
                    _ => {}
                }
            }
            ...
            job_skills_file
                .write_all(
                    format!(
                        ...
                        change_to_inflict_status_effects_grouped.len()
                    )
                    .as_bytes(),
                )
                .unwrap();
            for j in 0..change_to_inflict_status_effects_grouped.len() {
                ...
                for i in 1..=skill_config.max_level() {
                    job_skills_file
                        .write_all(format!("        if self.level == {} {{\n", i).as_bytes())
                        .unwrap();
                    for bonus in change_to_inflict_status_effects_grouped[j].iter() {
                        let mut write_inflict_effect = || match bonus.value.0 {
                            BonusType::ChanceToInflictStatusOnAttackPercentage(effect, chance) => {
                                job_skills_file
                                    .write_all(format!("            if chance <= {} {{\n", chance).as_bytes())
                                    .unwrap();
                                job_skills_file
                                    .write_all(format!("                effects.push(StatusEffect::{:?});\n", effect).as_bytes())
                                    .unwrap();
                                job_skills_file.write_all(b"            }\n").unwrap();
                            }
...
fn generate_bonuses(job_skills_file: &mut File, skill_config: &SkillConfig) {
    if skill_config.bonus_to_self().len() > 0 {
        job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
        job_skills_file
            .write_all(b"    fn _has_bonuses_to_self(&self) -> bool {\n")
            .unwrap();
        job_skills_file.write_all(b"        true\n").unwrap();
        job_skills_file.write_all(b"    }\n").unwrap();
        ...
        job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
        job_skills_file
            .write_all(b"    fn _bonuses_to_self(&self, tick: u128) -> TemporaryStatusBonuses {\n")
            .unwrap();
    ...
    if skill_config.bonus_to_target().len() > 0 {
        job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
        job_skills_file
            .write_all(b"    fn _has_bonuses_to_target(&self) -> bool {\n")
            .unwrap();
        job_skills_file.write_all(b"        true\n").unwrap();
        job_skills_file.write_all(b"    }\n").unwrap();

        job_skills_file
            .write_all(b"    fn _bonuses_to_target(&self, tick: u128) -> TemporaryStatusBonuses {\n")
            .unwrap();
    ...
    if skill_config.bonus_to_party().len() > 0 {
        job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
        job_skills_file
            .write_all(b"    fn _has_bonuses_to_party(&self) -> bool {\n")
            .unwrap();
        job_skills_file.write_all(b"        true\n").unwrap();
        job_skills_file.write_all(b"    }\n").unwrap();
        ...
        job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
        job_skills_file
            .write_all(b"    fn _bonuses_to_party(&self, tick: u128) -> TemporaryStatusBonuses {\n")
            .unwrap();
    ...
    if bonuses.len() > 0 {
        for i in 1..=skill_config.max_level() {
            job_skills_file
                ...
                .unwrap();
            job_skills_file
                .write_all(b"            return TemporaryStatusBonuses::new(vec![")
                .unwrap();
...
fn write_bonus(job_skills_file: &mut File, bonus: &BonusPerLevel, skill_config: &SkillConfig, level: Option<u32>) {
    ...
    let duration = if let Some(duration) = skill_config.duration1() {
        *duration
        ...
        println!(
            "No duration found for bonus for skill {}, will not generate bonus",
            skill_config.name
        );
        return;
    };
    job_skills_file
        .write_all(
            format!(
                ...
                bonus.value(),
                skill_config.bonus_flags().unwrap_or(StatusBonusFlag::Default.as_flag()),
                duration,
                skill_config.id
            )
            .as_bytes(),
...
fn generate_base_after_cast_walk_delay(job_skills_file: &mut File, skill_config: &SkillConfig) {
    ...
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn _base_after_cast_walk_delay(&self) -> u32 {\n")
        .unwrap();
    generate_return_per_level_u32(
        job_skills_file,
        skill_config.after_cast_walk_delay(),
        skill_config.after_cast_act_delay_per_level(),
    );
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn generate_base_after_cast_act_delay(job_skills_file: &mut File, skill_config: &SkillConfig) {
    ...
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn _base_after_cast_act_delay(&self) -> u32 {\n")
        .unwrap();
    generate_return_per_level_u32(
        job_skills_file,
        skill_config.after_cast_act_delay(),
        skill_config.after_cast_act_delay_per_level(),
    );
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn generate_as_offensive_skill(job_skills_file: &mut File) {
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn as_offensive_skill(&self) -> Option<&dyn OffensiveSkill> {\n")
        .unwrap();
...
fn generate_as_supportive_skill(job_skills_file: &mut File) {
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn as_supportive_skill(&self) -> Option<&dyn SupportiveSkill> {\n")
        .unwrap();
    job_skills_file.write_all(b"        Some(self)\n").unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn generate_is_interactive_skill(job_skills_file: &mut File) {
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn is_interactive_skill(&self) -> bool {\n")
        .unwrap();
    job_skills_file.write_all(b"        true\n").unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn generate_as_interactive_skill(job_skills_file: &mut File) {
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn as_interactive_skill(&self) -> Option<&dyn InteractiveSkill> {\n")
        .unwrap();
    job_skills_file.write_all(b"        Some(self)\n").unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn generate_as_ground_skill(job_skills_file: &mut File) {
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn as_ground_skill(&self) -> Option<&dyn GroundSkill> {\n")
        .unwrap();
    job_skills_file.write_all(b"        Some(self)\n").unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn generate_is_performance_skill(job_skills_file: &mut File) {
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn is_performance_skill(&self) -> bool {\n")
        .unwrap();
    job_skills_file.write_all(b"        true\n").unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn generate_as_performance_skill(job_skills_file: &mut File) {
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn as_performance_skill(&self) -> Option<&dyn PerformanceSkill> {\n")
        .unwrap();
    job_skills_file.write_all(b"        Some(self)\n").unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
}
...
fn generate_as_passive_skill(job_skills_file: &mut File) {
    job_skills_file.write_all(b"    #[inline(always)]\n").unwrap();
    job_skills_file
        .write_all(b"    fn as_passive_skill(&self) -> Option<&dyn PassiveSkill> {\n")
        .unwrap();
    job_skills_file.write_all(b"        Some(self)\n").unwrap();
    job_skills_file.write_all(b"    }\n").unwrap();
}
 ...
 *
 */
fn generate_skills_enum(
    output_path: &Path,
    skills: &Vec<SkillConfig>,
    skill_tree: &Vec<JobSkillTree>,
    _skills_already_generated: &BTreeSet<String>,
    _jobs_with_skills: &BTreeSet<String>,
) {
    ...
    for skill in skills.iter() {
        ...
        if let Some(class_name) = class_name {
            file.write_all(format!("    // {}: {}\n", class_name, skill.description).as_bytes())
                .unwrap();
        } else {
    ...
    for skill in skills.iter() {
        ...
        if let Some(effect_icon) = skill.effect_client_icon() {
            file.write_all(
                format!(
                    "            Self::{} => Some(ClientEffectIcon::{:?}),\n",
                    enum_name, effect_icon
                )
                .as_bytes(),
            )
            .unwrap();
...
fn generate_skills_enum_to_object(
    output_path: &Path,
    skills: &Vec<SkillConfig>,
    _skill_tree: &Vec<JobSkillTree>,
    skills_already_generated: &BTreeSet<String>,
    jobs_with_skills: &BTreeSet<String>,
) {
    let file_path = output_path.join("skill_enums.rs");
    let mut file = File::create(file_path.clone()).unwrap();
    write_file_header_comments(&mut file);
    ...
    for skill in skills.iter() {
        ...
        file.write_all(
            format!(
                "        SkillEnum::{} => {}::new(level).map(|s| Box::new(s) as Box<dyn Skill>),\n",
                to_enum_name(skill),
                to_struct_name(skill)
            )
            .as_bytes(),
        )
        .unwrap();
    ...
    for skill in skills.iter().filter(|skill_config| is_offensive(&skill_config)) {
        ...
        file.write_all(
            format!(
                "        SkillEnum::{} => {}::new(level).map(|s| Box::new(s) as Box<dyn OffensiveSkill>),\n",
                to_enum_name(skill),
                to_struct_name(skill)
            )
            .as_bytes(),
        ...
        fn $function(job_skills_file: &mut File, value: &Option<$type>, value_per_level: &Option<Vec<$type>>) {
            if let Some(value) = value {
                ...
                for (level, value_per_level) in value_per_level.iter().enumerate() {
                    ...
                    if $is_option {
                        job_skills_file
                            .write_all(format!("            return Some({:.3})\n", value_per_level).as_bytes())
                            .unwrap();
                    } else {
                        job_skills_file
                            .write_all(format!("            return {:.3}\n", value_per_level).as_bytes())
                            .unwrap();
                    }
                    job_skills_file.write_all(b"        }\n").unwrap();
                }
...
fn generate_validate_per_level(job_skills_file: &mut File, field_name: &str, value: &Option<u32>, value_per_level: &Option<Vec<u32>>) {
    if let Some(value) = value {
        job_skills_file
            .write_all(format!("        if {} > {} {{ Ok({}) }} else {{Err(())}}\n", field_name, value, value).as_bytes())
            .unwrap();
    } else if let Some(value_per_level) = value_per_level {
        for (level, value_per_level) in value_per_level.iter().enumerate() {
            if level == 0 {
            ...
            job_skills_file
                .write_all(
                    format!(
                        "            if {} >= {} {{ return Ok({}) }} else {{return Err(())}}\n",
                        field_name, value_per_level, value_per_level
                    )
                    .as_bytes(),
                )
                .unwrap();
        ...
        for skill in job_tree.tree().iter() {
            ...
            if skill.name().eq(skill_config.name())
                && job_tree
                    ...
                    .unwrap()
                    .to_lowercase()
                    .eq(skill.name().chars().nth(0).unwrap().to_lowercase())
            {
