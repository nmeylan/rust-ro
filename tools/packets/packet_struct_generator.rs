...
pub fn write_packets_struct(packets: Vec<PacketStructDefinition>, nested_structures: &Vec<StructDefinition>, output_path: &Path) {
    ...
    file_packets.write_all(b"#![allow(dead_code)]\n\n").unwrap();
    file_packets.write_all(b"use std::any::Any;\n\n").unwrap();

    file_packets_print.write_all(b"#![allow(dead_code)]\n\n").unwrap();
    file_packets_print.write_all(b"use crate::packets::*;\n").unwrap();
    file_packets_print.write_all(b"use std::fmt::{Formatter, Debug, Display};\n").unwrap();
    file_packets_print.write_all(b"use crate::print::PrettyOutput;\n\n").unwrap();
    ...
    file_packets_impl.write_all(b"use std::convert::TryInto;\n\n").unwrap();
...
fn write_file_header(file: &mut File) {
    file.write_all(b"// Generated by tools/packets_db/main.rs\n").unwrap();
...
fn write_packet_trait(file: &mut File) {
    file.write_all(b"pub trait Packet {\n").unwrap();
    file.write_all(b"    fn id(&self) -> &str;\n").unwrap();
    file.write_all(b"    fn display(&self);\n").unwrap();
    file.write_all(b"    fn debug(&self);\n").unwrap();
    file.write_all(b"    fn pretty_debug(&self);\n").unwrap();
...
fn write_default_trait_impl(file: &mut File, struct_definition: &StructDefinition) {
    ...
    file.write_all(b"    fn default() -> Self {\n\n").unwrap();
    file.write_all(b"       Self::new()\n").unwrap();
    file.write_all(b"    }\n").unwrap();
    file.write_all(b"}\n\n").unwrap();
}
...
fn write_debug_trait(file: &mut File, struct_definition: &StructDefinition, is_packet: bool) {
    ...
    file.write_all(b"    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {\n").unwrap();
    file.write_all(format!("        f.debug_struct(\"{}\")\n", struct_definition.name).as_bytes()).unwrap();
    if is_packet {
        file.write_all(b"            .field(\"id\", &self.id())\n").unwrap();
    }
...
fn write_display_trait(file: &mut File, struct_definition: &StructDefinition, _is_packet: bool) {
    ...
    file.write_all(b"    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {\n").unwrap();
    file.write_all(b"        let mut fields = Vec::new();\n").unwrap();
    for field in &struct_definition.fields {
        let value_to_print;
...
fn write_struct_definition(file: &mut File, struct_definition: &StructDefinition) {
    ...
    file.write_all(b"    pub raw: Vec<u8>,\n").unwrap();
    for field in &struct_definition.fields {
...
fn write_struct_from_method(file: &mut File, struct_definition: &StructDefinition) {
    ...
    file.write_all(format!("        {} {{\n", struct_definition.name).as_bytes()).unwrap();
    file.write_all(b"            raw: buffer.to_vec(),\n").unwrap();
    for field in &struct_definition.fields {
        ...
        if &field.data_type.name == "Vec" {
            file.write_all(format!("            {}_raw: vec_field.iter().map(|item| item.raw.clone()).collect::<Vec<Vec<u8>>>(),\n", field.name).as_bytes()).unwrap();
        } else if field.length > -1 {
            file.write_all(format!("            {}_raw: {{\n", field.name).as_bytes()).unwrap();
            file.write_all(format!("                let mut dst: [u8; {}] = [0u8; {}];\n", field.length, field.length).as_bytes()).unwrap();
            file.write_all(format!("                dst.clone_from_slice(&buffer[{}..{}]);\n", field.position, field.position + field.length as i16).as_bytes()).unwrap();
            file.write_all(b"                dst\n").unwrap();
            file.write_all(b"            },\n").unwrap();
        } else {
            file.write_all(format!("            {}_raw: buffer[{}..{}].to_vec(),\n", field.name, field.position, field_length(field)).as_bytes()).unwrap();
        }
    }
    file.write_all(b"        }\n").unwrap();
    file.write_all(b"    }\n").unwrap();
...
fn write_unknown_packet(file: &mut File) {
    ...
    file.write_all(b"pub struct PacketUnknown {\n").unwrap();
    file.write_all(b"    pub raw: Vec<u8>,\n").unwrap();
    file.write_all(b"    pub packet_id: String,\n").unwrap();
    file.write_all(b"}\n").unwrap();
    file.write_all(b"impl Packet for PacketUnknown {\n").unwrap();
    file.write_all(b"    fn id(&self) -> &str {\n").unwrap();
    file.write_all(b"        self.packet_id.as_str()\n").unwrap();
    file.write_all(b"    }\n").unwrap();
    file.write_all(b"    fn debug(&self) {\n").unwrap();
    file.write_all(b"            println!(\"{:?}\", self)\n").unwrap();
    file.write_all(b"    }\n").unwrap();
    file.write_all(b"    fn display(&self) {\n").unwrap();
    file.write_all(b"            self.debug()\n").unwrap();
    file.write_all(b"    }\n").unwrap();
    file.write_all(b"    fn pretty_debug(&self) {\n").unwrap();
    file.write_all(b"            self.debug()\n").unwrap();
    file.write_all(b"    }\n").unwrap();
    ...
    file.write_all(b"    pub fn from(buffer: &[u8]) -> PacketUnknown {\n").unwrap();
    file.write_all(b"        PacketUnknown { raw: buffer.to_vec(), packet_id: format!(\"0x{:02X?}{:02X?}\", buffer[0], buffer[1])}\n").unwrap();
    file.write_all(b"    }\n").unwrap();
    file.write_all(b"}\n").unwrap();
}
