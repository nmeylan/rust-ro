...
pub fn write_packets_struct(packets: Vec<PacketStructDefinition>, nested_structures: &Vec<StructDefinition>, output_path: &Path) {
    ...
    file_packets_print
        .write_all("#![allow(dead_code)]\n\n".to_string().as_bytes())
        .unwrap();
    file_packets_print
        .write_all("use crate::packets::*;\n".to_string().as_bytes())
        .unwrap();
    file_packets_print
        .write_all("use std::fmt::{Formatter, Debug, Display};\n".to_string().as_bytes())
        .unwrap();
    file_packets_print
        .write_all("use crate::print::PrettyOutput;\n\n".to_string().as_bytes())
        .unwrap();
    ...
    file_packets_impl
        ...
        .unwrap();
    file_packets_impl
        .write_all("#![allow(unused_assignments)]\n\n".to_string().as_bytes())
        .unwrap();
    file_packets_impl
    ...
    file_packets_impl
        ...
        .unwrap();
    file_packets_impl
        .write_all("use byteorder::{LittleEndian,WriteBytesExt};\n".to_string().as_bytes())
        .unwrap();
    file_packets_impl.write_all("use std::any::Any;\n".to_string().as_bytes()).unwrap();
    file_packets_impl
        .write_all("use std::convert::TryInto;\n\n".to_string().as_bytes())
        .unwrap();
...
fn write_packet_parser(file: &mut File, packets: &[PacketStructDefinition]) {
    ...
    packets.iter().for_each(|struct_def| {
        struct_def.ids.iter().for_each(|packet_id| {
            packets_with_version.push(PacketAndVersion {
                id: packet_id.id.clone(),
                version: packet_id.packetver,
    ...
    let mut ids_with_version = packets_with_version
        .iter()
        .filter(|packet| packet.version.is_some())
        .cloned()
        .collect::<Vec<PacketAndVersion>>();
    let ids_without_version = packets_with_version
        .iter()
        .filter(|packet| packet.version.is_none())
        .cloned()
        .collect::<Vec<PacketAndVersion>>();
    ...
    for packet in ids_with_version.iter() {
        ...
        file.write_all(
            format!(
                "    if packetver >= {} && buffer[0] == 0x{first_byte} && buffer[1] == 0x{second_byte} {{\n",
                packet.version.unwrap()
            )
            .as_bytes(),
        )
        .unwrap();
        file.write_all(format!("        return Box::new({}::from(buffer, packetver));\n", packet.struct_name).as_bytes())
            .unwrap();
        file.write_all("    }\n".to_string().as_bytes()).unwrap();
    }
    ...
    .unwrap();
    file.write_all(
        "    let entries: Vec<json_flat_parser::FlatJsonValue<&str>> = json_flat_parser::JSONParser::parse(json, \
         json_flat_parser::ParseOptions::default().keep_object_raw_data(false))?.json;\n"
            .as_bytes(),
    )
    .unwrap();
    ...
    for packet in ids_with_version.iter() {
        ...
        file.write_all(
            format!(
                "    if packetver >= {} && packet_id.value.unwrap().eq(\"{}\") {{\n",
                packet.version.unwrap(),
                packet_id
            )
            .as_bytes(),
        ...
        file.write_all(
            format!(
                "        return  {}::from_json(entries, packetver).map(|p| Box::new(p) as Box<dyn Packet>);\n",
                packet.struct_name
            )
            .as_bytes(),
        )
...
fn write_debug_trait(file: &mut File, struct_definition: &StructDefinition) {
    ...
    for field in &struct_definition.fields {
        file.write_all(
            format!(
                "            .field(\"{}[{}, {}]\", &format!(\"{{:02X?}}\", &self.{}_raw))\n",
                field.name,
                field.position,
                if field.length > -1 {
                    (field.position + field.length).to_string()
                } else {
                    "?".to_string()
                },
                field.name
...
fn write_display_trait(file: &mut File, struct_definition: &StructDefinition, _is_packet: bool) {
    ...
    for field in &struct_definition.fields {
        ...
        if field.data_type.name == "Array" {
            ...
            value_to_print = format!(
                "&self.{}.iter().map(|item| format!(\"\n  >{{}}\", item)).collect::<String>()",
                field.name
            );
        } else {
...
fn write_struct_from_method(file: &mut File, struct_definition: &StructDefinition) {
    file.write_all(
        format!(
            "    pub fn from(buffer: &[u8], packetver: u32) -> {} {{\n",
            struct_definition.name
        )
        .as_bytes(),
    )
    ...
    for field in &struct_definition.fields {
        if field.data_type.name == "Vec" {
            ...
            file.write_all(
                format!(
                    "                let field = {}::from(&buffer[offset..offset + {}::base_len(packetver)], packetver);\n",
                    field.complex_type.as_ref().unwrap(),
                    field.complex_type.as_ref().unwrap()
                )
                .as_bytes(),
            )
            ...
            if field.condition.is_some() {
                file.write_all(format!("                let field = {}", packetver_if("packetver", field)).as_bytes())
                    .unwrap();
                file.write_all(format!("                   {}\n", struct_impl_field_value(field)).as_bytes())
                    .unwrap();
                file.write_all("                } else {\n".to_string().as_bytes()).unwrap();
                file.write_all(format!("                   {}\n", struct_impl_default_field_value(field)).as_bytes())
                    .unwrap();
                file.write_all("                };\n".to_string().as_bytes()).unwrap();
            } else {
        ...
        if &field.data_type.name == "Vec" {
            ...
            file.write_all(format!("                let mut dst: [u8; {}] = [0u8; {}];\n", length, length).as_bytes())
                .unwrap();
            if field.condition.is_some() {
                file.write_all(format!("                {}", packetver_if("packetver", field)).as_bytes())
                    .unwrap();
                file.write_all(
                    format!(
                        "                    dst.clone_from_slice(&buffer[offset..offset + {}]);\n",
                        length
                    )
                    .as_bytes(),
                )
...
fn write_struct_fill_raw_with_packetver_method(file: &mut File, struct_definition: &StructDefinition, is_pub: bool) {
    ...
    for field in &struct_definition.fields {
        if field.data_type.name == "Vec" && field.complex_type.is_some() {
            file.write_all(
                format!(
                    "        self.{}.iter_mut().for_each(|item| wtr.append(&mut item.raw));\n",
                    field.name
                )
                .as_bytes(),
            )
            ...
            file.write_all(format!("          wtr.append(&mut self.{}_raw.to_vec());\n", field.name).as_bytes())
                .unwrap();
            file.write_all(format!("        }} else {}", packetver_if("packetver.unwrap()", field)).as_bytes())
                .unwrap();
            file.write_all(format!("            wtr.append(&mut self.{}_raw.to_vec());\n", field.name).as_bytes())
                .unwrap();
            file.write_all("        }\n".to_string().as_bytes()).unwrap();
...
fn write_struct_setter_methods(file: &mut File, struct_definition: &StructDefinition) {
    for field in &struct_definition.fields {
        ...
        file.write_all(
            format!(
                "    pub fn set_{}_raw(&mut self, value: {}) {{\n",
                field.name,
                field_type_raw(field)
            )
            .as_bytes(),
        )
        .unwrap();
...
fn write_struct_packet_id_method(file: &mut File, ids: &Option<Vec<PacketId>>) {
    ...
    if ids.len() == 1 {
        ...
        if ids_without_version.len() > 1 {
            panic!(
                "Cannot generate packet_id method when there is multiple id, and more than one(here {ids_without_version:?}) id has no \
                 version"
            );
        }
...
fn write_struct_new_method(file: &mut File, struct_definition: &StructDefinition, ids: &Option<Vec<PacketId>>) {
    ...
    if let Some(ids) = ids {
        ...
        if ids.len() == 1 {
            ...
            if ids_without_version.len() > 1 {
                panic!(
                    "Cannot generate new method when there is multiple id, and more than one(here {ids_without_version:?}) id has no \
                     version"
                );
            }
            ids_with_version.sort_by(|a, b| (b.packetver.unwrap()).cmp(&a.packetver.unwrap()));
            for (index, id) in ids_with_version.iter().enumerate() {
                ...
                if index == 0 {
                    file.write_all(
                        format!(
                            "        let (packet_id, packet_id_raw) = if packetver >= {} {{\n",
                            id.packetver.unwrap()
                        )
                        .as_bytes(),
                    )
                ...
                file.write_all(
                    format!("            (i16::from_le_bytes([0x{first_byte}, 0x{second_byte}]), [0x{first_byte}, 0x{second_byte}])\n")
                        .as_bytes(),
                )
                .unwrap();
            }
...
fn write_vec_field(file: &mut File, field: &StructField) {
    ...
    file.write_all(
        format!(
            "            vec_field.push({}::from(&buffer[start_pos..end_pos], packetver));\n",
            field.complex_type.as_ref().unwrap()
        )
        .as_bytes(),
    )
                ...
                "Array" => {
                    if field.sub_type.is_some() {
                        ...
                        if field.name.eq("pos_dir") || field.name.eq("dest") {
                            file.write_all(
                                format!(
                                    "        let x = ((self.{}[0] as u16) << 2) | (self.{}[1] >> 6) as u16;\n",
                            ...
                            file.write_all(
                                format!(
                                    "        let y = (((self.{}[1] & 0x3f) as u16) << 4) | (self.{}[2] >> 4) as u16;\n",
                                    field.name, field.name
                                )
                                .as_bytes(),
                            )
                            ...
                            json_value = format!(
                                "format!(\"\\\"{{}}\\\"\", {})",
                                format!("self.{}.iter().collect::<String>()", field.name)
                            );
                        } else {
                            json_value = format!(
                                "format!(\"[{{}}]\", {})",
                                format!(
                                    "self.{}.iter().map(|byte| byte.to_string()).collect::<Vec<String>>().join(\",\")",
                                    field.name
                                )
                            );
                        }
...
fn write_struct_from_json(file: &mut File, struct_definition: &StructDefinition) {
    ...
    file.write_all(
        "    pub fn from_json(entries: Vec<json_flat_parser::FlatJsonValue<&str>>, packetver: u32) -> Result<Self, String> {\n".as_bytes(),
    )
    .unwrap();
    ...
    for field in &struct_definition.fields {
            ...
            "Array" => {
                if let Some(sub_type) = field.sub_type {
                    if field.name.eq("pos_dir") || field.name.eq("dest") {
                        file.write_all(
                            format!(
                                "                let x: u16 = entries.iter().find(|entry| \
                                 entry.pointer.pointer.eq(\"/{}/x\")).unwrap().value.unwrap().parse().map_err(|_| \"Invalid value x for \
                                 {}\".to_string())?;\n",
                                field.name, field.name
                            )
                            .as_bytes(),
                        ...
                        file.write_all(
                            format!(
                                "                let y: u16 = entries.iter().find(|entry| \
                                 entry.pointer.pointer.eq(\"/{}/y\")).unwrap().value.unwrap().parse().map_err(|_| \"Invalid value y for \
                                 {}\".to_string())?;\n",
                                field.name, field.name
                            )
                            .as_bytes(),
                        ...
                        file.write_all(
                            format!(
                                "                let dir: u16 = entries.iter().find(|entry| \
                                 entry.pointer.pointer.eq(\"/{}/dir\")).unwrap().value.unwrap().parse().map_err(|_| \"Invalid value dir \
                                 for {}\".to_string())?;\n",
                                field.name, field.name
                            )
                            .as_bytes(),
        ...
        file.write_all(
            format!(
                "        if let Some(entry) = entries.iter().find(|entry| entry.pointer.pointer.eq(\"/{}\")) {{\n",
                field.name
            )
            .as_bytes(),
        )
        ...
        match field.data_type.name.as_str() {
            "Array" => {
                if let Some(sub_type) = field.sub_type {
                    ...
                    if field.name.eq("pos_dir") || field.name.eq("dest") {
                        ...
                        file.write_all(
                            format!(
                                "                let mut char_value: [char; {}] = [0 as char; {}];\n",
            ...
            "u8" | "i8" | "u16" | "i16" | "u32" | "i32" | "u64" | "i64" | "bool" => {
                file.write_all(
                    format!(
                        "                packet.set_{}(value.parse().map_err(|_| format!(\"Invalid value {{}} for {}\", value))?);\n",
                        field.name, field.name
                    )
                    .as_bytes(),
                )
...
fn struct_impl_field_value(field: &StructField) -> String {
    match field.data_type.name.as_str() {
        ...
        "u8" => String::from("u8::from_le_bytes([buffer[offset]])"),
        "i8" => String::from("i8::from_le_bytes([buffer[offset]])"),
        "u16" => String::from("u16::from_le_bytes([buffer[offset], buffer[offset + 1]])"),
        "i16" => String::from("i16::from_le_bytes([buffer[offset], buffer[offset + 1]])"),
        "u32" => String::from("u32::from_le_bytes([buffer[offset], buffer[offset + 1], buffer[offset + 2], buffer[offset + 3]])"),
        "i32" => String::from("i32::from_le_bytes([buffer[offset], buffer[offset + 1], buffer[offset + 2], buffer[offset + 3]])"),
        "u64" => String::from(
            "u64::from_le_bytes([buffer[offset], buffer[offset + 1], buffer[offset + 2], buffer[offset + 3], buffer[offset + 4], \
             buffer[offset + 5], buffer[offset + 6], buffer[offset + 7]])",
        ),
        "i64" => String::from(
            "i64::from_le_bytes([buffer[offset], buffer[offset + 1], buffer[offset + 2], buffer[offset + 3], buffer[offset + 4], \
             buffer[offset + 5], buffer[offset + 6], buffer[offset + 7]])",
        ),
        "bool" => String::from("buffer[offset] == 1"),
        "String" => "String::from_utf8_lossy(&buffer[offset..(buffer.len() - 1)]).to_string()".to_string(),
        "Array" => {
            let mut array_block = " {\n".to_string();
            ...
            if field.sub_type.is_some() && field.length > -1 {
                ...
                if sub_type_name == "char" {
                    array_block = format!(
                        "{array_block}                let mut dst: [{sub_type_name}; {length}] = [0 as {sub_type_name}; {length}];\n"
                    );
                } else {
                    array_block =
                ...
                let (read_array_entry, len) = match sub_type_name.as_str() {
                    "u8" => (String::from("u8::from_le_bytes([buffer[offset]])"), 1),
                    "i8" => (String::from("i8::from_le_bytes([buffer[offset]])"), 1),
                    "u16" => (String::from("u16::from_le_bytes([buffer[offset], buffer[offset + 1]])"), 2),
                    "i16" => (String::from("i16::from_le_bytes([buffer[offset], buffer[offset + 1]])"), 2),
                    "u32" => (
                    ...
                    "u64" => (
                        String::from(
                            "u64::from_le_bytes([buffer[offset], buffer[offset + 1], buffer[offset + 2], buffer[offset + 3], \
                    ...
                    "i64" => (
                        String::from(
                            "i64::from_le_bytes([buffer[offset], buffer[offset + 1], buffer[offset + 2], buffer[offset + 3], \
                             buffer[offset + 4], buffer[offset + 5], buffer[offset + 6], buffer[offset + 7]])",
                        ),
                        8,
                    ),
                    _ => {
                        panic!("Dont know of to read Vec<{}>", sub_type_name.as_str())
                    }
                };
        ...
        _ => {
            format!(
                "\"found unknown type {} for field {}. this won't compile!\"",
...
fn struct_impl_default_field_value(field: &StructField) -> String {
    match field.data_type.name.as_str() {
        "char" => String::from("0 as char"),
        "u8" => String::from("0 as u8"),
        "i8" => String::from("0 as i8"),
        "u16" => String::from("0 as u16"),
        "i16" => String::from("0 as i16"),
        "u32" => String::from("0 as u32"),
        "i32" => String::from("0 as i32"),
        "u64" => String::from("0 as u64"),
        "i64" => String::from("0 as i64"),
        "bool" => String::from("false"),
    ...
    match field.data_type.name.as_str() {
        ...
        "Array" => {
            if field.sub_type.is_some() {
                ...
                if sub_type.name == "u8" || sub_type.name == "i8" {
                    ...
                    res = format!("{}        for item in self.{} {{\n", res, field.name);
                    res = format!(
                        "{}            wtr.write_{}::<LittleEndian>(item).unwrap();\n",
                        res, sub_type.name
                    );
                    res = format!("{res}        }}\n");
                } else {
        ...
        "Vec" => {
            if field.complex_type.is_some() {
                ...
                res = format!(
                    "{}            self.{}.iter_mut().for_each(|item| item.fill_raw_with_packetver(packetver));\n",
                    res, field.name
                );
                res = format!(
                    "{}            self.{}.iter().map(|item| item.raw.clone()).collect()\n",
                    res, field.name
                );
                res = format!("{res}      }};\n");
                res
        ...
        "Struct" => {
            if field.complex_type.is_some() {
                ...
                if field.length > -1 {
                    res = format!(
                        "{}        self.{}_raw = self.{}.clone().raw.try_into().unwrap();\n",
                        res, field.name, field.name
                    );
                } else {
        ...
        "Array" => {
            if field.length > -1 {
                ...
                res = format!(
                    ...
                    res,
                    field.name,
                    field.length * field.sub_type.unwrap().length.unwrap()
                );
                res
            } else {
                res = format!("{}        {}: vec![],\n", res, field.name);
...
fn packetver_if(packetver_variable: &str, field: &StructField) -> String {
    format!("if {} {} {{\n", packetver_variable, match field.condition.as_ref().unwrap() {
