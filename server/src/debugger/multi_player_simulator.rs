...
impl MultiPlayerSimulator {
    ...
    pub fn simulate(&mut self, char_id: u32) {
        ...
        let session = Session::create_with_map_socket_and_char_id(
            account_id,
            char_id,
    ...
    pub fn replay_packet(&mut self, session_record: SessionRecord, sessions_index: Vec<usize>) {
        for session_index in sessions_index {
            info!(
                "Will replay session {} for session index {}",
                &session_record.entries_formatted, session_index
            );
            ...
            self.sender.send(SimlatorEvent::SessionReplay(
                session,
                session_record.clone(),
                self.server.state().characters().get(&char_i).as_ref().unwrap().name.clone(),
            ));
        }
    }
    ...
    pub fn stop_session_replay(&mut self, session_id: u32) {
        self.sessions
            .iter()
            .filter(|session| session.account_id == session_id)
            .for_each(|session| {
                self.sender.send(SimlatorEvent::SessionReplayStop(session.clone()));
            });
        self.session_replaying
            .retain(|replaying_session_id| *replaying_session_id != session_id);
    }
    ...
    pub fn simulation_loop(receiver: Receiver<SimlatorEvent>) {
        thread::Builder::new()
            .name("simulator-thread".to_string())
            .spawn(move || {
                        ...
                        if replay.next_entry_at <= tick {
                            ...
                            if let Some(entry) = entries_guard.get_mut(replay.current_entry) {
                                ...
                                if packet.as_any().downcast_ref::<PacketCzPlayerChat>().is_some() {
                                    ...
                                    let msg = orig_packet_player_chat
                                        .msg
                                        .replace("player :", format!("{} :", replay.char_name).as_str());
                                    ...
                                    debug!(
                                        ...
                                        packet_player_chat.name(),
                                        replay.current_entry
                                    );
                                    session
                                        .map_server_socket
                                        .as_ref()
                                        ...
                                        .write()
                                        .unwrap()
                                        .write_all(packet_player_chat.raw().as_slice());
                                } else {
