...
pub async fn main() {
    ...
    // Compiling item scripts into script VM bytecode, if needed (previous
    // compilation are stored in db, recompilation occurs when script has changed)
...
async fn compile_item_scripts(repository_arc: &Arc<PgRepository>, items: &mut Vec<ItemModel>) {
    ...
    for item in items.iter_mut() {
        if let Some(script) = &item.script {
            ...
            if item.script_compilation_hash.is_none() || script_hash != item.script_compilation_hash.unwrap() {
                let compilation_result = Compiler::compile_script_into_binary(
                    format!("itemscript{}", item.id),
                    script.as_str(),
    ...
    debug!("test log");
    repository_arc
        .update_script_compilation(script_compilation_to_update)
        .await
        .unwrap();
    info!(
        "Compiled {} item scripts compiled, skipped {} item scripts compilation (already compiled) in {}ms",
        item_script_compiled,
        item_script_skipped,
        start.elapsed().as_millis()
    );
}
