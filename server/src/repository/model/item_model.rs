...
use sqlx::{Database, Decode, Error, FromRow, Postgres, Row, TypeInfo};
...
impl<'r> FromRow<'r, PgRow> for ItemModel {
    fn from_row(row: &'r PgRow) -> Result<Self, Error> {
        ...
        if item_type == ItemType::Weapon {
            weapon_type = row
                .try_get("subtype")
                .or_else(Self::map_error::<Option<String>>())?
                .map(|s| WeaponType::from_string_ignore_case(s.as_str()));
        }
        ...
        if item_type == ItemType::Ammo {
            ammo_type = row
                .try_get("subtype")
                .or_else(Self::map_error::<Option<String>>())?
                .map(|s| AmmoType::from_string_ignore_case(s.as_str()));
        }
        ...
        row.try_get::<'r, Option<i16>, _>("job_all")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::All)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_acolyte")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Acolyte)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_alchemist")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Alchemist)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_archer")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Archer)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_assassin")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Assassin)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_barddancer")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
            ...
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_blacksmith")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Blacksmith)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_crusader")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Crusader)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_hunter")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Hunter)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_gunslinger")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Gunslinger)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_knight")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Knight)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_mage")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Mage)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_merchant")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Merchant)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_monk")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Monk)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_ninja")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Ninja)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_novice")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Novice)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_priest")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Priest)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_rogue")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Rogue)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_sage")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Sage)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_soullinker")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::SoulLinker)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_stargladiator")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::StarGladiator)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_supernovice")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::SuperNovice)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_swordman")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Swordsman)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_taekwon")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Taekwon)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_thief")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Thief)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("job_wizard")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    job_flags.push(EquipClassFlag::Wizard)
                }
            })
            .or_else(Self::map_error())?;
        ...
        row.try_get::<'r, Option<i16>, _>("class_all")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    class_flags.push(ItemClass::All)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("class_normal")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    class_flags.push(ItemClass::Normal)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("class_upper")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    class_flags.push(ItemClass::Upper)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("class_baby")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    class_flags.push(ItemClass::Baby)
                }
            })
            .or_else(Self::map_error())?;
        ...
        row.try_get::<'r, Option<i16>, _>("location_head_top")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::HeadTop)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_head_mid")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::HeadMid)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_head_low")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::HeadLow)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_armor")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::Armor)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_right_hand")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::HandRight)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_left_hand")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::HandLeft)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_garment")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::Garment)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_shoes")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::Shoes)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_right_accessory")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::AccessoryRight)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_left_accessory")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::AccessoryLeft)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_costume_head_top")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::CostumeHeadTop)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_costume_head_mid")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::CostumeHeadMid)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_costume_head_low")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::CostumeHeadLow)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_costume_garment")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::CostumeGarment)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_ammo")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::Ammo)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_shadow_weapon")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::ShadowWeapon)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_shadow_shield")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::ShadowShield)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_shadow_shoes")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::ShadowShoes)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_shadow_right_accessory")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::ShadowAccR)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("location_shadow_left_accessory")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    location_flags.push(EquipmentLocation::ShadowAccL)
                }
            })
            .or_else(Self::map_error())?;
        ...
        let mut flags = vec![];
        row.try_get::<'r, Option<i16>, _>("flag_buyingstore")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    flags.push(ItemFlag::BuyingStore)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("flag_deadbranch")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    flags.push(ItemFlag::DeadBranch)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("flag_container")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
            ...
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("flag_uniqueid")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    flags.push(ItemFlag::UniqueId)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("flag_bindonequip")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    flags.push(ItemFlag::BindOnEquip)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("flag_dropannounce")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    flags.push(ItemFlag::DropAnnounce)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("flag_noconsume")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    flags.push(ItemFlag::NoConsume)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("flag_dropeffect")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    flags.push(ItemFlag::DropEffect)
                }
            })
            .or_else(Self::map_error())?;
        ...
        row.try_get::<'r, Option<i16>, _>("trade_nodrop")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    trade_flags.push(ItemTradeFlag::NoDrop)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("trade_notrade")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    trade_flags.push(ItemTradeFlag::NoTrade)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("trade_tradepartner")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    trade_flags.push(ItemTradeFlag::TradePartner)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("trade_nosell")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    trade_flags.push(ItemTradeFlag::NoSell)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("trade_nocart")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    trade_flags.push(ItemTradeFlag::NoCart)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("trade_nostorage")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    trade_flags.push(ItemTradeFlag::NoStorage)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("trade_noguildstorage")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    trade_flags.push(ItemTradeFlag::NoGuildStorage)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("trade_noauction")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    trade_flags.push(ItemTradeFlag::NoAuction)
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<i16>, _>("trade_nomail")
            .map(|v| {
                if v.is_some() && v.unwrap() != 0 {
                    trade_flags.push(ItemTradeFlag::NoMail)
                }
            })
            .or_else(Self::map_error())?;
        ...
        row.try_get::<'r, Option<Vec<u8>>, _>("script_compilation")
            .map(|v| {
                if let Some(v) = v {
                    script_compilation = Some(general_purpose::STANDARD.encode(v))
                }
            })
            .or_else(Self::map_error())?;
        row.try_get::<'r, Option<Vec<u8>>, _>("script_compilation_hash")
            .map(|v| {
                if let Some(v) = v {
                    let hash: [u8; 16] = v.try_into().unwrap();
                    script_compilation_hash = Some(u128::from_le_bytes(hash))
                }
            })
            .or_else(Self::map_error())?;
...
impl ItemModel {
    ...
    fn enum_flags_into_u64<T: EnumWithMaskValueU64>(flags: &Vec<T>) -> u64 {
        flags.iter().fold(0, |acc, enum_flag| acc | enum_flag.as_flag())
    }
...
impl<'r> Decode<'r, Postgres> for DBItemType {
    fn decode(value: <Postgres as Database>::ValueRef<'r>) -> Result<Self, BoxDynError> {
        ...
        Ok(DBItemType {
            item_type: ItemType::from_string(value),
        })
    }
}
