...
impl InventoryRepository for PgRepository {
    async fn character_inventory_update_add(
        ...
        let updated_item_ids_amounts: Vec<(i32, i16)> = inventory_update_items
            .iter()
            .map(|item| (item.item_id, item.amount))
            .collect();
        tx.execute(
            sqlx::query(
                "INSERT INTO inventory (char_id, nameid, amount, identified) \
        ...
        tx.execute(
            sqlx::query(
                "INSERT INTO inventory (char_id, nameid, amount, identified, unique_id) \
        ...
        if buy {
            ...
            let cost = updated_item_ids_amounts
                .iter()
                .fold(0, |mut acc, (id, amount)| {
            ...
            let updated_zeny = tx
                .fetch_all(
                    sqlx::query(
                        "UPDATE char set zeny = zeny - $1 WHERE char_id = $2 RETURNING zeny;",
                    )
                    .bind(cost)
    ...
    async fn character_inventory_delete(
        &self,
    ...
    async fn character_inventory_fetch(
        &self,
        char_id: i32,
    ...
    async fn character_inventory_wearable_item_update(
        &self,
        items: Vec<InventoryItemModel>,
    ) -> Result<PgQueryResult, Error> {
    ...
    async fn character_slot_card(
        ...
        // Try to find an empty card slot and update it, returning which slot was updated
        ...
        debug!("character_slot_card statement params: {}, {}, {}", card_inventory_item.item_id, equipment_inventory_item.id, char_id);
