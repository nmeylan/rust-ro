...
impl ItemRepository for PgRepository {
    async fn item_buy_sell_fetch_all_where_ids(&self, ids: Vec<i32>) -> Result<Vec<ItemBuySellModel>, Error> {
        sqlx::query_as(
            "SELECT id, type, price_buy, price_sell, stack_amount, weight, name_english FROM item_db item WHERE id IN (SELECT * FROM \
             UNNEST($1::int4[])) ORDER BY id",
        )
        .bind(ids)
        .fetch_all(&self.pool)
        .await
    ...
    async fn get_items(&self, ids_or_names: Vec<Value>) -> Result<Vec<GetItemModel>, Error> {
        let mut names = vec![];
        let mut ids = vec![];
        ids_or_names.iter().for_each(|id| match id {
            Value::String(v) => names.push(v.clone()),
            Value::Number(v) => ids.push(*v),
        });
        sqlx::query_as::<_, GetItemModel>(
            "SELECT id, type, 0::int2 as amount, weight, name_english, name_aegis FROM item_db item WHERE id IN (SELECT * FROM \
             UNNEST($1::int4[])) OR name_aegis ILIKE ANY($2::text[])",
        )
        .bind(ids)
        .bind(names)
        .fetch_all(&self.pool)
