...
impl Repository {
    ...
    pub async fn get_items(&self, ids: Vec<i32>, amounts: Vec<i16>) -> Result<Vec<GetItemModel>, Error> {
        sqlx::query_as("SELECT id, type, (select $2::int2[])[row_number() OVER ()] as amount, weight, name_english FROM item_db as amount WHERE id IN (SELECT * FROM UNNEST($1::int4[]))")
            .bind(ids)
            .bind(amounts).fetch_all(&self.pool).await
    }
}