...
fn before_each_with_latch(latch_size: usize) -> BattleServiceTestContext {
    ...
    BattleServiceTestContext {
        ...
        battle_min_service: BattleService::new(
            client_notification_sender.clone(),
            StatusService::instance(),
            GlobalConfigService::instance(),
            BattleResultMode::TestMin,
        ),
        battle_max_service: BattleService::new(
            client_notification_sender.clone(),
            StatusService::instance(),
            GlobalConfigService::instance(),
...
mod tests {
    ...
    use crate::tests::common::character_helper::{
        create_character, equip_item_from_id, equip_item_from_id_with_cards, equip_item_from_name,
    };
    ...
    fn test_damage_character_attack_mob_melee() {
        ...
        for stat in stats {
            ...
            let min = context
                .battle_min_service
                .calculate_damage(status_snapshot!(context, character), &mob.status, None);
            let max = context
                .battle_max_service
                .calculate_damage(status_snapshot!(context, character), &mob.status, None);
            // Then
            assert!(
                stat.min_damage - 1 <= min && min <= stat.min_damage + 1,
                "Expected min damage to be {} but was {} with stats {:?}",
                stat.min_damage,
                min,
                stat
            );
            assert!(
                stat.max_damage - 1 <= max && max <= stat.max_damage + 1,
                "Expected max damage to be {} but was {} with stats {:?}",
                stat.max_damage,
                max,
                stat
            );
    ...
    fn test_damage_weapon_attack_should_depend_on_dex() {
        ...
        for stat in stats {
            ...
            // When
            equip_item_from_name(&mut character, stat.weapon);
            let min = context.battle_min_service.weapon_atk(
                status_snapshot!(context, character),
                &context.status_service.to_snapshot(&target_status),
                false,
            );
            let max = context.battle_max_service.weapon_atk(
                status_snapshot!(context, character),
                &context.status_service.to_snapshot(&target_status),
                false,
            );
    ...
    fn test_attack_when_repeat_attack_is_true_should_not_clear_attack() {
        ...
        // When
        let attack_1 = context.battle_max_service.basic_attack(
            &mut character,
            mob.to_map_item_snapshot(),
            character_status,
            &mob.status,
            get_tick(),
        );
        let attack_2 = context.battle_max_service.basic_attack(
            &mut character,
            mob.to_map_item_snapshot(),
            character_status,
            &mob.status,
        ...
        assert_eq!(
            character.attack.unwrap().last_attack_motion,
            context.status_service.attack_motion(character_status)
        );
    }
    ...
    fn test_attack_should_not_attack_if_there_is_delay() {
        ...
        // When
        let character_status = status_snapshot!(context, character);
        let attack_1 = context.battle_service.basic_attack(
            &mut character,
            mob.to_map_item_snapshot(),
            character_status,
            &mob.status,
        ...
        let attack_2 = context.battle_service.basic_attack(
            &mut character,
            mob.to_map_item_snapshot(),
            character_status,
            &mob.status,
    ...
    fn test_size_modifier() {
        ...
        let scenario = vec![
            Scenarii {
                weapon: None,
                target_size: Size::Small,
                expected_modifier: 1.0,
            },
            Scenarii {
                weapon: None,
                target_size: Size::Medium,
                expected_modifier: 1.0,
            },
            Scenarii {
                weapon: None,
                target_size: Size::Large,
                expected_modifier: 1.0,
            },
            Scenarii {
                weapon: Some("Knife"),
        ...
        for scenarii in scenario {
            ...
            let size_modifier = BattleService::size_modifier(
                status_snapshot!(context, character),
                &context.status_service.to_snapshot(&target_status),
            );
            assert_eq!(
                size_modifier, scenarii.expected_modifier,
                "Expected size modifier to be {} but was {} when weapon is {:?} and target size {:?}",
                scenarii.expected_modifier, size_modifier, scenarii.weapon, target_status.size
            )
        }
    }
    ...
    fn test_mob_vitdef() {
        ...
        for vit in [0, 1, 2, 45, 88].iter() {
            ...
            // Then
            assert!(
                actual_vit >= (*vit as f32),
                "Expected actual_vit {} to be greater or equal to {}",
                actual_vit,
                vit
            );
        }
    ...
    fn test_vitdef() {
        ...
        let scenario = vec![
            Scenarii {
                item: "Knife",
                vit: 99,
                expected_vitdef_min: 78,
                expected_vitdef_max: 113,
            },
            Scenarii {
                item: "Knife",
                vit: 49,
                expected_vitdef_min: 38,
                expected_vitdef_max: 39,
            },
        ];
        // When
        for scenarii in scenario {
    ...
    fn test_attack_element() {
        ...
        let scenario = vec![
            Scenarii {
                weapon: "Sword",
                ammo: None,
                expected_element: Element::Neutral,
                skill: None,
            },
            Scenarii {
                weapon: "Fire_Brand",
            ...
            Scenarii {
                weapon: "Katar_Of_Piercing_Wind",
                ammo: Some("Fire_Arrow"),
                expected_element: Element::Wind,
                skill: None,
            },
            Scenarii {
                weapon: "Katar_Of_Thornbush",
                ammo: None,
        ...
        for scenari in scenario {
            ...
            let element = context
                .battle_service
                .attack_element(status_snapshot!(context, character), scenari.skill.as_deref());
            assert_eq!(
                element,
                scenari.expected_element,
                "expected {} but got {} for weapon {} and skill {:?}",
                scenari.expected_element,
                element,
                scenari.weapon,
                scenari.skill.map(|s| s.id())
            );
        }
    }
    ...
    fn test_attack_element_modifiers() {
        ...
        battle_test_cases(
            ...
            &context.battle_min_service,
            &context.battle_max_service,
        )
    }
    ...
    fn battle_test_cases(
        ...
        result_file
            .write_all(b"|id|character|stats|skill|target|passed|min dmg|max dmg|\n")
            .unwrap();
        result_file.write_all(b"|-|-|-|-|-|-|-|-|\n").unwrap();
        ...
        for result in results.iter_mut() {
            ...
            let min_dmg_passed = result.expected.min_dmg() - 1 <= result.actual_combat_result.as_ref().unwrap().min_dmg
            ...
            if result.expected.skill_to_use().skid() > 0 {
                skill = format!(
                    "{} (Lv. {})",
                    SkillEnum::from_id(result.expected.skill_to_use().skid()).to_name(),
