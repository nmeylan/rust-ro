...
impl PlayerScriptHandler {
    pub fn handle_shop(&self, native: &Native, params: Vec<Value>, execution_thread: &Thread, call_frame: &CallFrame) -> bool {
        if native.name.eq("callshop") {
            ...
            self.send_packet_to_char(self.session.account_id, &mut packet_zc_select_deal_type);
            ...
            self.send_packet_to_char(self.session.account_id, &mut packet_zc_pc_purchase_itemlist);
