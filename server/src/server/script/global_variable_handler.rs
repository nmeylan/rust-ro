...
use crate::server::script::{
    GlobalVariableEntry, GlobalVariableScope, PlayerScriptHandler, VM_THREAD_CONSTANT_INDEX_ACCOUNT_ID, VM_THREAD_CONSTANT_INDEX_CHAR_ID,
};
...
impl PlayerScriptHandler {
    pub fn handle_setglobalvariable(&self, params: &[Value], execution_thread: &Thread) {
        ...
        match variable_scope.as_str() {
            "char_permanent" => {
                ...
                if value.is_number() {
                    self.server.repository.script_variable_char_num_save(
                        char_id,
                        variable_name.to_string(),
                        0,
                        value.number_value().unwrap(),
                    );
                } else {
                    self.server.repository.script_variable_char_str_save(
                        char_id,
                        variable_name.to_string(),
                        0,
                        value.string_value().unwrap().clone(),
                    );
                }
            }
            "account_permanent" => {
                if value.is_number() {
                    self.server.repository.script_variable_account_num_save(
                        execution_thread.get_constant(VM_THREAD_CONSTANT_INDEX_ACCOUNT_ID),
                        variable_name.to_string(),
                        0,
                        value.number_value().unwrap(),
                    );
                } else {
                    self.server.repository.script_variable_account_str_save(
                        execution_thread.get_constant(VM_THREAD_CONSTANT_INDEX_ACCOUNT_ID),
                        variable_name.to_string(),
                        0,
                        value.string_value().unwrap().clone(),
            ...
            "server_permanent" => {
                if value.is_number() {
                    self.server
                        .repository
                        .script_variable_server_num_save(variable_name.to_string(), 0, value.number_value().unwrap());
                } else {
                    self.server.repository.script_variable_server_str_save(
                        variable_name.to_string(),
                        0,
                        value.string_value().unwrap().clone(),
                    );
            ...
            "char_temporary" => {
                ...
                let value = match value {
                    ...
                    _ => {
                        panic!("Can't store any variable other than Number or String")
                    }
                };
        ...
        match variable_scope.as_str() {
            "char_permanent" => {
                ...
                if variable_name.ends_with('$') {
                    execution_thread.push_constant_on_stack(Value::new_string(self.server.repository.script_variable_char_str_fetch_one(
                        char_id,
                        variable_name.clone(),
                        0,
                    )));
                } else {
                    execution_thread.push_constant_on_stack(Value::new_number(self.server.repository.script_variable_char_num_fetch_one(
                        char_id,
                        variable_name.clone(),
                        0,
                    )));
            ...
            "account_permanent" => {
                ...
                if variable_name.ends_with('$') {
                    execution_thread.push_constant_on_stack(Value::new_string(
                        self.server
                            .repository
                            .script_variable_account_str_fetch_one(account_id, variable_name.clone(), 0),
                    ));
                } else {
                    execution_thread.push_constant_on_stack(Value::new_number(
                        self.server
                            .repository
                            .script_variable_account_num_fetch_one(account_id, variable_name.clone(), 0),
                    ));
                }
            }
            "server_permanent" => {
                if variable_name.ends_with('$') {
                    execution_thread.push_constant_on_stack(Value::new_string(
                        self.server
                            .repository
                            .script_variable_server_str_fetch_one(variable_name.clone(), 0),
                    ));
                } else {
                    execution_thread.push_constant_on_stack(Value::new_number(
                        self.server
                            .repository
                            .script_variable_server_num_fetch_one(variable_name.clone(), 0),
                    ));
                }
            }
            "char_temporary" => {
    ...
    pub fn handle_setglobalarray(&self, params: &Vec<Value>, execution_thread: &Thread) {
        ...
        loop {
            ...
            match variable_scope.as_str() {
                ...
                "server_permanent" => {
                    if value.is_number() {
                        self.server.repository.script_variable_server_num_save(
                            variable_name.to_string(),
                            array_index as u32,
                            value.number_value().unwrap(),
                        );
    ...
    pub fn handle_remove_item_from_globalarray(&self, params: &[Value], execution_thread: &Thread) {
        ...
        if variable_scope == "char_temporary" {
            ...
            for i in start_index..end_index {
                script_variable_store.remove_global_by_name_and_scope_and_index(
                    variable_name,
                    &GlobalVariableScope::CharTemporary,
                    i as usize,
                );
            }
        } else {
            error!(
                "handle_remove_item_from_globalarray not supported yet for scope {}",
                variable_scope
            );
        };
    }
            ...
            "char_permanent" => {
                ...
                if variable_name.ends_with('$') {
                    let rows = self
                        .server
                        .repository
                        .script_variable_char_str_fetch_all(char_id, variable_name.clone());
                    Self::push_array_str_elements_on_stack(execution_thread, rows);
                } else {
                    let rows = self
                        .server
                        .repository
                        .script_variable_char_num_fetch_all(char_id, variable_name.clone());
            ...
            "account_permanent" => {
                ...
                if variable_name.ends_with('$') {
                    let rows = self
                        .server
                        .repository
                        .script_variable_account_str_fetch_all(account_id, variable_name.clone());
                    Self::push_array_str_elements_on_stack(execution_thread, rows);
                } else {
                    let rows = self
                        .server
                        .repository
                        .script_variable_account_num_fetch_all(account_id, variable_name.clone());
                    Self::push_array_num_elements_on_stack(execution_thread, rows);
                }
            }
            "server_permanent" => {
    ...
    fn store_special_char_variable(&self, char: &Character, special_variable_name: &str, value: &Value) -> bool {
        match special_variable_name {
            "Zeny" => {
                self.server.add_to_next_tick(CharacterUpdateZeny(CharacterZeny {
                    char_id: char.char_id,
                    zeny: Some(value.number_value().unwrap() as u32),
                }));
                true
            }
            &_ => false,
