use packets::packets::{
    PacketZcCloseDialog, PacketZcMenuList, PacketZcNotifyPlayerchat, PacketZcNpcChat, PacketZcOpenEditdlg, PacketZcOpenEditdlgstr,
...
impl PlayerInteractionScriptHandler {
    pub fn handle_interaction(&self, native: &Native, params: &Vec<Value>, execution_thread: &Thread, call_frame: &CallFrame) -> bool {
        if native.name.eq("mes") {
            ...
            packet_dialog.msg = params
                .iter()
                .map(|text| text.string_value().unwrap().clone())
                .collect::<Vec<String>>()
                .join("\n");
            packet_dialog.naid = execution_thread.get_constant(VM_THREAD_CONSTANT_INDEX_NPC_ID);
            packet_dialog.packet_length =
                (PacketZcSayDialog::base_len(self.server.packetver()) as i16 + packet_dialog.msg.len() as i16) + 1_i16;
            packet_dialog.fill_raw();
            self.send_packet_to_char(
            ...
            self.send_packet_to_char(
                execution_thread.get_constant(VM_THREAD_CONSTANT_INDEX_CHAR_ID),
                &mut packet_zc_show_image2,
            );
            return true;
        } else if native.name.eq("purchaseitems") {
            let (owner_reference, reference) = params[0]
                .reference_value()
                .map_err(|err| execution_thread.new_runtime_from_temporary(err, "purchaseitems first argument should be array reference"))
                .unwrap();
            let items_ids_array = execution_thread.vm.array_from_heap_reference(owner_reference, reference).unwrap();
            let (owner_reference, reference) = params[1]
                .reference_value()
                .map_err(|err| execution_thread.new_runtime_from_temporary(err, "purchaseitems second argument should be array reference"))
                .unwrap();
            let items_amount_array = execution_thread.vm.array_from_heap_reference(owner_reference, reference).unwrap();
            let items_amounts: Vec<i16> = execution_thread
                .array_constants(items_amount_array)
                .iter()
                .map(|constant| *constant.value().number_value().as_ref().unwrap() as i16)
                .collect::<Vec<i16>>();
            let mut items_ids_amount: Vec<(crate::server::script::Value, i16)> = vec![];
            execution_thread
                .array_constants(items_ids_array)
                .iter()
                .enumerate()
                .for_each(|(i, constant)| {
                    if constant.value().is_number() {
                        // TODO handle string
                        items_ids_amount.push((
                            crate::server::script::Value::Number(constant.value().number_value().unwrap()),
                            items_amounts[i],
                        ))
                    } else {
                        items_ids_amount.push((
                            crate::server::script::Value::String(constant.value().string_value().unwrap().clone()),
                            items_amounts[i],
                        ))
                    }
                });

            self.server.script_service().schedule_get_items(
                execution_thread.get_constant(VM_THREAD_CONSTANT_INDEX_CHAR_ID),
                &self.runtime,
                items_ids_amount,
                true,
            ...
            let (owner_reference, reference) = params[0]
                .reference_value()
                .map_err(|err| execution_thread.new_runtime_from_temporary(err, "sellitems first argument should be array reference"))
                .unwrap();
            let items_ids_array = execution_thread.vm.array_from_heap_reference(owner_reference, reference).unwrap();
            let (owner_reference, reference) = params[1]
                .reference_value()
                .map_err(|err| execution_thread.new_runtime_from_temporary(err, "sellitems second argument should be array reference"))
                .unwrap();
            let items_amount_array = execution_thread.vm.array_from_heap_reference(owner_reference, reference).unwrap();
            let items_amounts: Vec<i16> = execution_thread
                .array_constants(items_amount_array)
            ...
            let (owner_reference, reference) = params[2]
                .reference_value()
                .map_err(|err| execution_thread.new_runtime_from_temporary(err, "sellitems third argument should be array reference"))
                .unwrap();
            let items_price_array = execution_thread.vm.array_from_heap_reference(owner_reference, reference).unwrap();
            let items_prices: Vec<i16> = execution_thread
                .array_constants(items_price_array)
                .iter()
                .map(|constant| *constant.value().number_value().as_ref().unwrap() as i16)
                .collect::<Vec<i16>>();
            ...
            execution_thread
                ...
                .for_each(|(i, constant)| {
                    if constant.value().is_number() {
                        items_to_remove.push(CharacterRemoveItem {
                            char_id,
                            index: constant.value().number_value().unwrap() as usize,
                            amount: items_amounts[i],
                            price: items_prices[i] as i32,
                        })
                    }
                });
            ...
            self.runtime.block_on(async {
                let items_ids: Vec<i32> = execution_thread
                    .array_constants(items_ids_array.clone())
                    .iter()
                    .map(|constant| *constant.value().number_value().as_ref().unwrap())
                    .collect::<Vec<i32>>();
                let items_amounts: Vec<i16> = execution_thread
                    .array_constants(items_amount_array)
                    .iter()
                ...
                self.server
                    ...
                    .for_each(|(id, weight)| {
                        items_total_weight += weight
                            * (items_ids_amount
                                .iter()
                                .find(|(iid, _amount)| *iid == *id)
                                .unwrap_or(&(*id, 0_i16))
                                .1 as i32)
                    });
                let character_ref = self
                    .server
                    .state()
                    .get_character_unsafe(execution_thread.get_constant(VM_THREAD_CONSTANT_INDEX_CHAR_ID));
                execution_thread.push_constant_on_stack(value::Value::new_number(
                    if self
                        .server
                        .character_service()
                        .can_carry_weight(character_ref, items_total_weight as u32)
                    {
                        1
                    } else {
                        0
            ...
            packet_zc_notify_playerchat
                .set_packet_length((PacketZcNotifyPlayerchat::base_len(self.server.packetver()) + message.len() + 1) as i16);
            packet_zc_notify_playerchat.fill_raw();
            self.send_packet_to_char(
                execution_thread.get_constant(VM_THREAD_CONSTANT_INDEX_CHAR_ID),
                &mut packet_zc_notify_playerchat,
            );
    ...
    fn handle_menu(&self, execution_thread: &Thread, params: &Vec<value::Value>) -> Option<usize> {
        ...
        packet_zc_menu_list.packet_length =
            (PacketZcMenuList::base_len(self.server.packetver()) as i16 + packet_zc_menu_list.msg.len() as i16) + 1_i16;
        packet_zc_menu_list.fill_raw();
        self.send_packet_to_char(
            execution_thread.get_constant(VM_THREAD_CONSTANT_INDEX_CHAR_ID),
            &mut packet_zc_menu_list,
        );
