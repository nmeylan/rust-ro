...
impl PlayerInteractionScriptHandler {
    pub fn new(
        ...
        server: Arc<Server>,
        player_action_receiver: RwLock<Receiver<Vec<u8>>>,
        runtime: Arc<Runtime>,
...
impl PlayerInteractionScriptHandler {
    ...
    pub(crate) fn send_packet_to_char(&self, char_id: u32, packet: &mut dyn Packet) {
        self.client_notification_channel
            ...
            .expect("Failed to send packet to char");
    }
}
...
impl NativeMethodHandler for PlayerInteractionScriptHandler {
    fn handle(
        ...
        self.player_script_handler
            .handle(native, params, execution_thread, call_frame, source_line, class_name);
    }
}
...
impl NativeMethodHandler for PlayerScriptHandler {
    fn handle(
        ...
        if native.name.eq("print") {
            ...
            self.server.add_to_next_tick(CharacterUpdateLook(CharacterLook {
                look_type: LookType::from_value(look_type as usize),
                look_value: look_value as u16,
                char_id,
            }));
            ...
            self.server.server_service.schedule_warp_to_walkable_cell(
                self.server.state_mut().as_mut(),
                map_name,
                x as u16,
                y as u16,
                char_id,
            ...
            let nums = params[1..params.len()]
                .iter()
                .map(|p| if p.is_number() { Some(p.number_value().unwrap()) } else { None })
            ...
            self.server.script_skill_service().handle_skill(
            ...
            execution_thread.push_constant_on_stack(value::Value::new_string(
                JobName::from_value(job_number as usize).as_str().to_string(),
            ));
            ...
            let (is_male, mask) = if !params.is_empty() {
                let is_male = if params.len() == 2 {
                    !params[1]
                        .string_value()
                        .expect("Expected roclass argument 1 to be a string with value 'm' or 'f'")
                        .to_lowercase()
                        .eq("f")
                } else {
                    true
            ...
            execution_thread.push_constant_on_stack(value::Value::new_number(
                JobName::from_mask(mask as u64, is_male).map_or(-1, |job| job.value() as i32),
            ));
            ...
            if let Ok(job) = JobName::try_from_value(job as usize) {
                self.server.add_to_next_tick(GameEvent::CharacterChangeJob(CharacterChangeJob {
                    char_id,
                    job,
