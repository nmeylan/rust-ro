...
impl GlobalConfigService {
    ...
    pub fn init(
        ...
        SERVICE_INSTANCE_INIT.call_once(|| unsafe {
            ...
            SERVICE_INSTANCE = Some(GlobalConfigService {
                configuration,
    ...
    pub fn get_job_config(&self, id: u32) -> &JobConfig {
        self.jobs
            ...
            .unwrap_or_else(|| panic!("Expected to find job config for id {id} but found none"))
    }
    ...
    pub fn get_skill_config_by_name(&self, name: &str) -> &SkillConfig {
        let id = self
            .skills_name_id
            .get(name)
            .unwrap_or_else(|| panic!("Expected to find skill config for name {name} but found none"));
        self.skills
            .get(id)
            .unwrap_or_else(|| panic!("Expected to find skill config for name {name} but found none"))
    }
    ...
    pub fn get_skill_config(&self, id: u32) -> &SkillConfig {
        self.skills
            .get(&id)
            .unwrap_or_else(|| panic!("Expected to find skill config for id {id} but found none"))
    }
    ...
    pub fn get_item_id_from_name(&self, name: &str) -> u32 {
        *self
            .items_name_id
            .get(name)
            .unwrap_or_else(|| panic!("Expected to find item for name {name} but found none"))
    }
    ...
    pub fn get_mob_id_from_name(&self, name: &str) -> u32 {
        *self
            .mobs_name_id
            .get(name)
            .unwrap_or_else(|| panic!("Expected to find mob for name {name} but found none"))
    }
