...
impl CharacterService {
    ...
    pub fn print(&self, character: &Character) {
        ...
        writeln!(
            stdout,
            "************** {} - {} ****************",
            character.name, character.char_id
        )
        .unwrap();
        ...
        let mut inventory_print = |predicate: PredicateClosure| {
            character.inventory_iter().filter(predicate).for_each(|(index, item)| {
                writeln!(
                    stdout,
                    " [{}] {} - {} ({})",
                    index, item.name_english, item.item_id, item.amount
                )
                .unwrap()
            });
        };
    ...
    pub fn change_map(&self, new_map_instance_key: &MapInstanceKey, new_position: Position, character: &mut Character) {
        ...
        self.client_notification_sender
            .send(Notification::Char(CharNotification::new(
                character.char_id,
                std::mem::take(packet_zc_npcack_mapmove.raw_mut()),
            )))
    ...
    pub fn update_zeny(&self, runtime: &Runtime, zeny_update: CharacterZeny, character: &mut Character) {
        let zeny = if let Some(zeny) = zeny_update.zeny {
            ...
            runtime.block_on(async {
                self.repository
                    .character_zeny_fetch(zeny_update.char_id)
                    .await
                    .expect("failed to fetch zeny") as u32
            })
        };
        ...
        self.client_notification_sender
            .send(Notification::Char(CharNotification::new(
                character.char_id,
                std::mem::take(packet_zc_longpar_change.raw_mut()),
            )))
    ...
    pub fn sit(&self, character: &mut Character) {
        ...
        self.client_notification_sender
            .send(Notification::Char(CharNotification::new(
                character.char_id,
                chain_packets(vec![&packet_zc_msg_state_change, &packet_zc_notify_act3]),
            )))
    ...
    pub fn regen_hp(&self, character: &mut Character, tick: u128) {
            ...
            && character_status.hp() < character_status.max_hp()
        {
            ...
            self.client_notification_sender
                .send(Notification::Char(CharNotification::new(
                    character.char_id,
                    chain_packets(vec![&packet_status_hp_change]),
                )))
    ...
    pub fn regen_sp(&self, character: &mut Character, tick: u128) {
            ...
            && character_status.sp() < character_status.max_sp()
        {
            ...
            self.client_notification_sender
                .send(Notification::Char(CharNotification::new(
                    character.char_id,
                    chain_packets(vec![&packet_status_sp_change]),
                )))
    ...
    pub async fn save_characters_state(&self, characters: Vec<&Character>, tick: u128) {
        ...
        for character in characters {
            self.repository
                .character_save_temporary_bonus(character.char_id, character.account_id, &character.status.temporary_bonuses)
                .await
    ...
    pub fn notify_weight(&self, character: &Character) {
        self.client_notification_sender
            .send(Notification::Char(CharNotification::new(
                character.char_id,
                self.weight_update_packets(character),
            )))
    ...
    pub fn load_temporary_bonuses_from_db(&self, runtime: &Runtime, character: &mut Character) {
        let result = runtime.block_on(async {
            self.repository
                .character_load_temporary_bonus(character.char_id, character.account_id)
                .await
        });
    ...
    pub fn update_base_level(&self, character: &mut Character, maybe_new_base_level: Option<u32>, maybe_level_delta: Option<i32>) -> i32 {
        ...
        let new_base_level = if let Some(new_base_level) = maybe_new_base_level {
            ...
            ((old_base_level as i32 + add_level)
                .min(self.configuration_service.config().game.max_base_level as i32)
                .max(1)) as u32
        } else {
            old_base_level
        };
        character.status.base_level = new_base_level;
        self.update_status_point(
            character,
            (character.status.status_point as i32 + self.calculate_status_point_delta(old_base_level, new_base_level)).max(0) as u32,
        );
    ...
    pub fn get_job_level_max(&self, character: &mut Character) -> u32 {
        self.configuration_service
            .get_job_config(character.status.job)
            .job_level()
            .max_job_level() as u32
    }
    ...
    pub fn update_job_level(&self, character: &mut Character, maybe_new_base_level: Option<u32>, maybe_level_delta: Option<i32>) -> i32 {
        ...
        let new_job_level = if let Some(new_job_level) = maybe_new_base_level {
            ...
            ((old_job_level as i32 + add_level)
                .min(self.get_job_level_max(character) as i32)
                .max(1)) as u32
        } else {
            old_job_level
        };
        ...
        if new_job_level > old_job_level {
            ...
            let skill_points = if self.should_reset_skills(character) {
    ...
    fn stat_raising_cost_for_next_level(&self, level: u16, stat_name: &str) -> u32 {
        self.configuration_service
            .config()
            .game
            .status_point_raising_cost
            .iter()
            .find(|status_point_raising_cost| status_point_raising_cost.level_min <= level && level <= status_point_raising_cost.level_max)
            .map(|status_point_raising_cost| status_point_raising_cost.raising_cost as u32)
            .unwrap_or_else(|| {
    ...
    pub fn stat_value(&self, status: &Status, status_type: &StatusTypes) -> u16 {
        match status_type {
            StatusTypes::Str => status.str,
            StatusTypes::Agi => status.agi,
            StatusTypes::Vit => status.vit,
            StatusTypes::Int => status.int,
            StatusTypes::Dex => status.dex,
            StatusTypes::Luk => status.luk,
            _ => {
    ...
    pub fn stat_mut<'a>(&'a self, status: &'a mut Status, status_type: &StatusTypes) -> &'a mut u16 {
        match status_type {
            StatusTypes::Str => &mut status.str,
            StatusTypes::Agi => &mut status.agi,
            StatusTypes::Vit => &mut status.vit,
            StatusTypes::Int => &mut status.int,
            StatusTypes::Dex => &mut status.dex,
            StatusTypes::Luk => &mut status.luk,
            _ => {
    ...
    pub fn allocate_status_point(&self, character: &mut Character, status_type: StatusTypes, value_to_add: u16) -> bool {
        ...
        // With this calculation method, when value_to_add is > 1, like 10 and there is
        // in theory enough status point available to add 3 points to the stat,
    ...
    pub fn calculate_status_point_delta(&self, from_level: u32, to_level: u32) -> i32 {
        ...
        for i in start..end {
            status_point_count += self
                .configuration_service
                .config()
                .game
                .status_point_rewards
                .iter()
                .find(|status_point_reward| status_point_reward.level_min as u32 <= i && i <= status_point_reward.level_max as u32)
                .map(|status_point_reward| status_point_reward.reward as i32)
                .unwrap_or_else(|| {
    ...
    pub fn reset_stats(&self, character: &mut Character) {
        ...
        self.update_status_point(
            character,
            self.get_status_point_count_for_level(character) - self.get_spent_status_point(character),
        );
        for (column_name, _) in &[
            ("str", StatusTypes::Str.value() as u16),
            ("agi", StatusTypes::Agi.value() as u16),
            ("dex", StatusTypes::Dex.value() as u16),
            ("vit", StatusTypes::Vit.value() as u16),
            ("int", StatusTypes::Int.value() as u16),
            ("luk", StatusTypes::Luk.value() as u16),
        ] {
            self.persistence_event_sender
                .send(PersistenceEvent::UpdateCharacterStatusU32(StatusUpdate {
                    char_id: character.char_id,
    ...
    pub fn next_base_level_required_exp(&self, status: &Status) -> u32 {
        ...
        if status.base_level > exp.len() as u32 {
            // TODO: define a formula in configuration for infinite leveling, which increase
            // leveling curve, level after level. For now disable exp gain when
            // nothing is defined
            u32::MAX
        } else {
    ...
    pub fn reload_client_side_status(&self, character: &Character) {
        ...
        let mut final_response_packet: Vec<u8> = chain_packets(vec![
            ...
            &packet_status_point,
            &packet_skill_point,
            &packet_str_increase_cost,
            &packet_agi_increase_cost,
            &packet_vit_increase_cost,
            &packet_dex_increase_cost,
            &packet_int_increase_cost,
            &packet_luk_increase_cost,
            &packet_hit,
            &packet_flee,
            &packet_aspd,
            &packet_atk,
            ...
            &packet_sp,
            &packet_speed,
            &packet_exp_required_to_reach_next_base_level,
            &packet_exp_required_to_reach_next_job_level,
        ]);
    ...
    pub fn load_units_in_fov(&self, server_state: &ServerState, character: &mut Character, map_instance_state: &MapInstanceState) {
                ...
                if matches!(map_item.object_type(), MapItemType::DroppedItem) {
                    ...
                    if let Some(mob) = map_instance_state.get_mob(map_item.id()) {
                        ...
                        if mob.is_moving() {
                            ...
                            // packet_zc_notify_move.move_data =
                            // mob_movement.from.to_move_data(&mob_movement.to);
                            packet_zc_notify_move.set_move_start_time(get_tick_client());
                            packet_zc_notify_move.fill_raw();
                            #[cfg(feature = "debug_mob_movement")]
                            {
                                info!(
                                    "A moving mob appeared! {} moving from {} to {}.",
                                    mob.id,
                                    mob.position(),
                                    mob.movements.first().unwrap().position()
                                );
                            }
                    ...
                    if let Some(other_character) = server_state.get_character(map_item.id()) {
                        ...
                        if let Some(i) = other_character.status.head_low() {
                            packet_zc_notify_standentry
                                .set_accessory(GlobalConfigService::instance().get_item(i.item_id).view.unwrap() as u16)
                        }
                        if let Some(i) = other_character.status.head_mid() {
                            packet_zc_notify_standentry
                                .set_accessory3(GlobalConfigService::instance().get_item(i.item_id).view.unwrap() as u16)
                        }
                        if let Some(i) = other_character.status.head_top() {
                            packet_zc_notify_standentry
                                .set_accessory2(GlobalConfigService::instance().get_item(i.item_id).view.unwrap() as u16)
                        }
                        ...
                        // packetver());
                        //     packet_zc_notify_move.set_gid(other_character.
                        // id);
                        //     packet_zc_notify_move.move_data =
                        // other_character.position().
                        // to_move_data(other_character.movements.first().
                        // unwrap().position());     // packet_zc_notify_move.
                        // move_data =
                        // mob_movement.from.to_move_data(&mob_movement.to);
                        //     packet_zc_notify_move.
                        // set_move_start_time(get_tick_client());
                        //     packet_zc_notify_move.fill_raw();
                        //     #[cfg(feature = "debug_mob_movement")]
                        //     {
                        //         info!("A moving mob appeared! {} moving from
                        // {} to {}.", other_character.id,
                        // other_character.position(),
                        // other_character.movements.first().unwrap().
                        // position());     }
                        //     packets.extend(packet_zc_notify_move.raw);
                        // }
                    }
        ...
        if !packets.is_empty() {
            self.client_notification_sender
                .send(Notification::Char(CharNotification::new(character.char_id, packets)))
    ...
    pub fn character_increase_stat(&self, character: &mut Character, character_update_stat: CharacterUpdateStat) {
        let result = self.allocate_status_point(
            character,
            StatusTypes::from_value(character_update_stat.stat_id as usize),
            character_update_stat.change_amount,
        );
        ...
        packet_zc_status_change_ack.set_value(self.stat_value(
            &character.status,
            &StatusTypes::from_value(character_update_stat.stat_id as usize),
        ) as u8);
        ...
        self.client_notification_sender
            .send(Notification::Char(CharNotification::new(
                character.char_id,
                packet_zc_status_change_ack.raw,
            )))
            .unwrap_or_else(|_| {
                error!("Failed to send notification packet_zc_status_change_ack(character_increase_stat update) to client")
            });
    }

    pub fn character_kill_monster(
        &self,
        character: &mut Character,
        character_kill_monster: CharacterKillMonster,
    ...
    pub fn character_join_map_effect(&self, character: &Character) {
        ...
        packet_zc_notify_newentry3
            .set_packet_length(PacketZcNotifyStandentry5::base_len(GlobalConfigService::instance().packetver()) as i16);
        packet_zc_notify_newentry3.set_job(character.status.job as i16);
        packet_zc_notify_newentry3.set_sex(character.sex);
        packet_zc_notify_newentry3.set_pos_dir(
            Position {
                x: character.x,
                y: character.y,
                dir: character.dir,
        ...
        if let Some(i) = character.status.head_low() {
            packet_zc_notify_newentry3.set_accessory(GlobalConfigService::instance().get_item(i.item_id).view.unwrap() as i16)
        }
        if let Some(i) = character.status.head_mid() {
            packet_zc_notify_newentry3.set_accessory3(GlobalConfigService::instance().get_item(i.item_id).view.unwrap() as i16)
        }
        if let Some(i) = character.status.head_top() {
            packet_zc_notify_newentry3.set_accessory2(GlobalConfigService::instance().get_item(i.item_id).view.unwrap() as i16)
        }
        ...
        self.client_notification_sender
            .send(Notification::Area(AreaNotification::from_character_exclude_self(
                character,
                packet_zc_notify_newentry3.raw,
            )))
    ...
    pub fn cancel_movement(&self, character: &mut Character, tick: u128) {
        ...
        self.client_notification_sender
            .send(Notification::Area(AreaNotification::from_character(
                character,
                std::mem::take(packet_zc_stop_move.raw_mut()),
            )))
