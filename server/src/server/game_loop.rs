...
impl Server {
            ...
            for task in tasks {
                match task {
                    ...
                    GameEvent::CharacterChangeMap(event) => {
                        ...
                        server_ref
                            .character_service()
                            .change_map(map_instance.key(), event.new_position.unwrap(), character);
    ...
    pub(crate) fn character_movement_loop(
        server_ref: Arc<Server>,
        client_notification_sender_clone: SyncSender<Notification>,
