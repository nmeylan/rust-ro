...
impl Debug for Notification {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Notification::Char(n) => {
                let packet = if !n.packet.is_empty() {
                    format!("0x{:02X?}{:02X?}", n.packet[0], n.packet[1])
                } else {
                    "empty".to_string()
                };
            ...
            Notification::Area(n) => f
                .debug_struct("AreaNotification")
...
impl Notification {
    pub fn packet(&self, packetver: u32) -> Box<dyn Packet> {
        match self {
            Notification::Char(notification) => packets_parser::parse(&notification.packet[..], packetver),
            Notification::Area(notification) => packets_parser::parse(&notification.packet[..], packetver),
        }
    }
}
...
pub enum AreaNotificationRangeType {
    ...
    }, // Notify all players in the FoV of the point.
}
    ...
    pub fn from_character_exclude_self(character: &Character, packets: Vec<u8>) -> Self {
        Self {
            ...
            range_type: AreaNotificationRangeType::Fov {
                x: character.x(),
                y: character.y(),
                exclude_id: Some(character.char_id),
            },
            packet: packets,
        }
    }
