...
use crate::ast::statement_list::*;
use crate::ast::statement::*;
use crate::ast::compound_statement::*;
use crate::ast::expression_statement::*;
use crate::ast::primary_expression::*;
use crate::ast::postfix_expression::*;
use crate::ast::assignment_expression_todo::*;
...
pub fn parse(tokens: Arc<Vec<Token>>) {
    ...
    while !parser_state.is_at_end(){
        parse_root_expression(&mut parser_state);
    }
}
...
pub fn parse_root_expression(parser_state: &mut ParserState) -> Result<RootExpression, String> {
    ...
    let beginning_parser_token = parser_state.current_token;
    if result.is_err() {
        parser_state.set_current_token(beginning_parser_token);
        result = (|| {
        let children_node_0_result = parse_statement_list(parser_state);
        let children_node_0 = children_node_0_result?;

        let expression = RootExpression::build_from_statement_list(Box::new(children_node_0.clone()));
        println!("{:?}", expression);
...
pub fn parse_statement_list(parser_state: &mut ParserState) -> Result<StatementList, String> {
    let mut result: Result<StatementList, String>;
    result = Err("Haven't match (todo)".to_string());
    let beginning_parser_token = parser_state.current_token;
    if result.is_err() {
        parser_state.set_current_token(beginning_parser_token);
        result = (|| {
        let children_node_0_result = parse_statement(parser_state);
        let children_node_0 = children_node_0_result?;

        let expression = StatementList::build_from_statement(Box::new(children_node_0.clone()));
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
    if result.is_err() {
        ...
        let children_node_1_result = parse_statement(parser_state);
        let children_node_1 = children_node_1_result?;

        let expression = StatementList::build_from_statement_list1(Box::new(children_node_0.clone()), Box::new(children_node_1.clone()));
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
...
pub fn parse_statement(parser_state: &mut ParserState) -> Result<Statement, String> {
    ...
    if result.is_err() {
        parser_state.set_current_token(beginning_parser_token);
        result = (|| {
        let children_node_0_result = parse_compound_statement(parser_state);
        let children_node_0 = children_node_0_result?;

        let expression = Statement::build_from_compound_statement(Box::new(children_node_0.clone()));
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
    if result.is_err() {
        parser_state.set_current_token(beginning_parser_token);
        result = (|| {
        let children_node_0_result = parse_expression_statement(parser_state);
        let children_node_0 = children_node_0_result?;

        let expression = Statement::build_from_expression_statement(Box::new(children_node_0.clone()));
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
...
// "LeftBracket" <statement_list> "RightBracket"
pub fn parse_compound_statement(parser_state: &mut ParserState) -> Result<CompoundStatement, String> {
    let mut result: Result<CompoundStatement, String>;
    result = Err("Haven't match (todo)".to_string());
    let beginning_parser_token = parser_state.current_token;
    if result.is_err() {
        parser_state.set_current_token(beginning_parser_token);
        result = (|| {
        let token_parse_0_result = parse_token(parser_state, TokenType::LeftBracket);
        let token_parse_0 = token_parse_0_result?;

        let children_node_1_result = parse_statement_list(parser_state);
        let children_node_1 = children_node_1_result?;

        let token_parse_2_result = parse_token(parser_state, TokenType::RightBracket);
        let token_parse_2 = token_parse_2_result?;

        let expression = CompoundStatement::build_from_compound_statement0(token_parse_0, Box::new(children_node_1.clone()), token_parse_2);
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
...
// <any_expression> | "SemiColon"
pub fn parse_expression_statement(parser_state: &mut ParserState) -> Result<ExpressionStatement, String> {
    let mut result: Result<ExpressionStatement, String>;
    result = Err("Haven't match (todo)".to_string());
    let beginning_parser_token = parser_state.current_token;
    if result.is_err() {
        parser_state.set_current_token(beginning_parser_token);
        result = (|| {
        let children_node_0_result = parse_any_expression(parser_state);
        let children_node_0 = children_node_0_result?;

        let expression = ExpressionStatement::build_from_any_expression(Box::new(children_node_0.clone()));
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
    if result.is_err() {
        parser_state.set_current_token(beginning_parser_token);
        result = (|| {
        let token_parse_0_result = parse_token(parser_state, TokenType::SemiColon);
        let token_parse_0 = token_parse_0_result?;

        let expression = ExpressionStatement::build_from_semi_colon(token_parse_0);
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
...
pub fn parse_assignment_expression_todo(parser_state: &mut ParserState) -> Result<AssignmentExpressionTodo, String> {
    let mut result: Result<AssignmentExpressionTodo, String>;
    result = Err("Haven't match (todo)".to_string());
    let beginning_parser_token = parser_state.current_token;
    if result.is_err() {
        parser_state.set_current_token(beginning_parser_token);
        ...
        let expression = AssignmentExpressionTodo::build_from_conditional_expression(Box::new(children_node_0.clone()));
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
    if result.is_err() {
        ...
        let expression = AssignmentExpressionTodo::build_from_assignment_expression_todo1(Box::new(children_node_0.clone()), Box::new(children_node_1.clone()), Box::new(children_node_2.clone()));
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
    if result.is_err() {
        ...
        let expression = AssignmentExpressionTodo::build_from_unary_expression(Box::new(children_node_0.clone()));
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
...
// <unary_expression> <assignment_operator> <assignment_expression> | <unary_expression>
pub fn parse_assignment_expression(parser_state: &mut ParserState) -> Result<AssignmentExpression, String> {
    ...
    if result.is_err() {
        ...
        let expression = AssignmentExpression::build_from_assignment_expression0(Box::new(children_node_0.clone()), Box::new(children_node_1.clone()), Box::new(children_node_2.clone()));
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
...
// <variable> "Equal" <assignment_expression> | <variable>
pub fn parse_declaration_specifiers(parser_state: &mut ParserState) -> Result<DeclarationSpecifiers, String> {
    ...
    if result.is_err() {
        ...
        let expression = DeclarationSpecifiers::build_from_declaration_specifiers0(Box::new(children_node_0.clone()), token_parse_1, Box::new(children_node_2.clone()));
        println!("{:?}", expression);
        return Ok(expression)
        })();
    }
