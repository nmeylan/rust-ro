...
pub fn stackable_enum(input: TokenStream) -> TokenStream {
    ...
    if let Enum(enum_data) = &input.data {
        let get_value_sum_arms = enum_data
            .variants
            .iter()
            .filter(|variant| matches!(&variant.fields, syn::Fields::Unnamed(..)))
            .map(|variant| {
                let variant_name = variant.ident.clone();
                if let syn::Fields::Unnamed(fields) = &variant.fields {
                    let value_offset = if variant.attrs.iter().find(|attr| attr.path().is_ident("value_offset")).is_some() {
